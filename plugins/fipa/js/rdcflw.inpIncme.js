...
var rdcflwIncAssTbl;
/*var rdcflwIncAssTbl=$('#generateproIncometbl').DataTable( {
...
//aravindh
$("#txtFldDlgprojIRAgePaySts").on("change",function(){//aravindh
	if(!rcpfStartAgeValidate($('#txtFldRDSlfIntAge'),$("#txtFldDlgprojIRAgePaySts")))return;//aravindh
	if(!rdStartAgeValidate($('#txtFldDlgprojIRAgePaySts'),$('#selDlgprojIRAgeBsOn')))return;//aravindh
});
...
$("#txtFldDlgprojIncAssAgePaySts").on("change",function(){//aravindh
	if(!rcpfStartAgeValidate($('#txtFldRDSlfIntAge'),$("#txtFldDlgprojIncAssAgePaySts")))return;//aravindh
	if(!rdStartAgeValidate($('#txtFldDlgprojIncAssAgePaySts'),$('#selDlgprojIncAssAgeBsOn')))return;//aravindh
});		
		
RDInctbl = $('#RDInctbl').DataTable( {
	destroy: true,
  	responsive: false,         
     ordering: false,
     searching: false,     
...
function RDincretfilldomval($RowId,$row){
	...
	$row.find("td:eq(0)").find('input:eq(1)').val($('#txtFldDlgprojIRId').val());
...
//	if(!validationRetirementPlnSection())return;
			...
			loadAgeStartEnd('txtFldDlgORAgePaySts',$("#selDlgORAgeBsOn"));
			//var intretslfage=Number($("#txtFldRDSlfIntAge").val());//aravindh
			//$('#ProjOfIncAss_Dialog #txtFldDlgprojIncAssAgePaySts').val(intretslfage);//aravindh
	 ...
	 $("#RDInctbl tbody tr").each(function(j,row){
		 	...
		 	var crntdesc=$(this).find("td:eq(3)").find("input:eq(0)").val(); 
		 	 ...
		 	 if(mode_r != "D"){
			 		...
			 		income_data.data=crntdesc+"-a-"+rind;
			 		income_data.title=crnthd +"<br/><hr/>"+crntdesc ;//+"_"+cola;
			 		income_data.width="15px";
			 		income_data.className="dt-right";
					income_data.render= $.fn.dataTable.render.number(',', '.', 0, '$');
			 	
			 		headlist.push(crnthd+"-a-"+rind);
			 		subheadlist.push(crntdesc+"-a-"+rind);
			 		dataHeader.push(income_data); 
...
//			 	cola=subheadlist.length;
	...
	// var asststartAge;//aravindh131221
    // var startAgearr=[];//aravindh131221
	 if(asstblcount>0){
...
//		 var crntId=$(this).find("td:eq(0)").find("input:eq(2)").val();
		// asststartAge =$(this).find("td:eq(9)").find("input:eq(0)").val();//aravindh131221
		 if(mode_r != "D"){
			 
			 var income_data = {}; 
			 income_data.data=crntdesc+"-b-"+rind;
			 income_data.title=crnthd +"<br/><hr/>"+crntdesc ;//+"_"+cola;
			 income_data.width="15px";
			 income_data.className="dt-right";
			 income_data.render= $.fn.dataTable.render.number(',', '.', 0, '$');
	 	
			 headlist.push(crnthd+"-b-"+rind);//income_data
			 ...
			 cola=subheadlist.length;
	 		// startAgearr.push(asststartAge);//aravindh131221
		}
	 ...
	 /*Data Set*/
	 //var selfage=Math.min.apply(null, startAgearr) // aravindh131221
	 var intretslfage=Number($("#txtFldRDSlfIntAge").val());
	 var intretspsage=Number($("#txtFldRDSpsIntAge").val());

	 var totAge=Number($("#txtFldRDSlfProjLfe").val());
	 //var selfStartage=(selfage < intretslfage)?selfage:intretslfage;// aravindh131221
	 //var intretslfage=selfStartage;// aravindh131221
 	// var intretspsage=selfStartage;// aravindh131221
	 var dataset=[];
                    ...
                    tooltips: {
                        ...
                        callbacks: {
                            ...
                            label: (tooltipItem, data) => {
                                return data.datasets[tooltipItem.datasetIndex].label + ": $ " + tooltipItem.yLabel;//number format 092021
                            }
...
function RDincAssretfilldomval($RowId,$row){
	$row.find("td:eq(2)").find('input:eq(1)').val($("#txtFldDlgprojIncAssId").val());
...
function syncToRpcf2(){
	 ...
	 var totAge=Number($("#retSelfProjage").val());
		...
		'<input type="hidden" name="txtFldprojIRCrtdBy"/><input type="hidden" name="txtFldprojIRCrtdDate"/>'; 
		 ...
		 // this table is hidden in retirement planning section so we can comment this block to avoid unknown issue - 092021
	/*
		 ...
		 if($rowCount >0){
			 ...
			 $("#IncAssRetPlgtbl tbody tr").each(function(i,row){  
				 ...
				 if(mode_r != "D"){
					...
					var description  = $(this).find("td:eq(2)").find('input[name=txtFldIncAssDesc]').val();
					var clasification  = $(this).find("td:eq(3)").find('select[name=txtFldIncAssClsfy]').val();
					var subclasification  = $(this).find("td:eq(3)").find('input[name=txtFldIncAsstSubClsfy]').val();
					var amountofincome = $(this).find("td:eq(4)").find('input[name=txtFldIncAssAmtofInc]').val();
					var frequency = $(this).find("td:eq(4)").find('select[name=selIncAssFreq]').val();
					var esclationrate = $(this).find("td:eq(4)").find('input[name=txtFldIncAssEslrate]').val();
					var roirate =  $(this).find("td:eq(4)").find('input[name=txtFldIncAssRoi]').val();
					
					var paystart = $(this).find("td:eq(5)").find('input[name=txtFldIncAssAgePaySts]').val();
					var payend = $(this).find("td:eq(5)").find('input[name=txtFldIncAssAgePayends]').val();
					var basedon = $(this).find("td:eq(5)").find('select[name=selIncAssAgeBsOn]').val();
					
					frequency = isEmpty(frequency) ? "REGULAR" : frequency;
					amountofincome = (isEmpty(amountofincome)? Number("0") :  Number(amountofincome))
					esclationrate = (isEmpty(esclationrate)? Number("0") :  Number(esclationrate))
					roirate = (isEmpty(roirate)? Number("0") :  Number(roirate))
					paystart = (isEmpty(paystart)? intretslfage : Number(paystart))
					payend =(isEmpty(payend)? totAge : Number(payend))
					clasification = isEmpty(clasification) ? subclasification : (clasification + " " +subclasification);
					...
					$lastRow.find("td:eq(3)").find('input:eq(0)').val(description);
					...
					$lastRow.find("td:eq(5)").find('input:eq(0)').val(amountofincome).addClass("applyEvntUsd");
					$lastRow.find("td:eq(6)").find('input:eq(0)').val(esclationrate).addClass("applyEvntpCent3");
					$lastRow.find("td:eq(7)").find('input:eq(0)').val(roirate).addClass("applyEvntpCent3");
					var iassagebsed =  $("#selDlgIncAssAgeBsOn > option").clone();
					$lastRow.find("td:eq(8)").find('select:eq(0)').append(iassagebsed);
					$lastRow.find("td:eq(8)").find('select:eq(0)').val(basedon);
					$lastRow.find("td:eq(9)").find('input:eq(0)').val(paystart);
					$lastRow.find("td:eq(10)").find('input:eq(0)').val(payend);
