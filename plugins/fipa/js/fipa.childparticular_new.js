/*$(document).on("click", "#Childdtlstablbl", function () {
		 $("#poptxtFldNewChldYrs2ter").hover(function() {
	         $(this).prop('popShown',true).popover('show');
	    },function(){ $(this).prop('popShown',true).popover('hide');});
...
function getNewChildRows(dataset){
	...
	$(".dates").trigger('click');$(".dates").trigger('click');
	$(document).on('click',".dates ",function(){
		 $(this).datetimepicker(dobOptions);  
    });

	
		 $(".childTerInfo").hover(function() {
	         $(this).prop('popShown',true).popover('show');
	    },function(){ $(this).prop('popShown',true).popover('hide');});
...
function addNewChildNameDyn(elmid,mode){
	 ...
	 if(!isEmpty(elmid)){
		// clearExistChildName();
		 ...
		 $("#lipAssured option[value='"+elmid+"']").remove();
		 ...
		 $("#Cob_Dialog #txtFldDlgMainAccHolderName option[value='"+elmid+"']").remove(); 
		 //Life Ins Education Plg
		 /*$("#liEducationPlg_Dialog #selDlgEdPlgChldName option[value='"+elmid+"']").remove();*/
		 $("#selDlgEdPlgChldName option[value='"+elmid+"']").remove();
...
function addNewChildNameDynEle(childarr,mode){
	 ...
	 if(!isEmpty(elmid)){
		// clearExistChildName();
		 ...
		 for(var childr=0;childr<childarr.length;childr++) {
				
				var childname = childarr [childr];
				if(!isEmpty(childname)){
					 
					 $("#InptInvstDets_Dialog #txtFldDlgInvstChildName option[value='"+childname+"']").remove(); 
					 $("#lipAssured option[value='"+childname+"']").remove();
					 $("#Cob_Dialog #selDlgCOBChildName option[value='"+childname+"']").remove();
					 $("#Investment_Dialog #selDlgInvNameOfChild option[value='"+childname+"']").remove();
					 $("#selDlgCPFNameOfChild  option[value='"+childname+"']").remove();
					 $("#Cob_Dialog #txtFldDlgMainAccHolderName option[value='"+childname+"']").remove(); 
					 $("#selDlgEdPlgChldName option[value='"+childname+"']").remove();
					 $("#dlgli_EducationPlg_tab #txtFldDlgChildName option[value='"+childname+"']").remove();
...
//	var inptinsLen= $("#listofLifeIns_Dialog #lipAssured option").length;  
	...
	var lifeInsChildNamelen= $("#selDlgEdPlgChldName option").length; 
...
//	 }
	 ...
	 for(var k=1;k<=lifeInsChildNamelen;k++){
		/* $("#liEducationPlg_Dialog #selDlgEdPlgChldName option").remove(0);*/
		 $("#selDlgEdPlgChldName option").remove(0);
	 }   
...
//	if(!isEmpty(spousename)){$('#listofLifeIns_Dialog #lipAssured').append( '<option value="'+spousename+'">'+spousename+'</option>' );}
	...
	$("#selDlgEdPlgChldName").append( '<option value="">--SELECT--</option>' ); 
	...
	if(rowCount >0){
		$('#childParticularsTableNew tbody').find('tr:visible').each(function(){
			...
			if (childNameArr.includes(childname) === false) childNameArr.push(childname);
			
			
		}); 
		
		for(var childr=0;childr<childNameArr.length;childr++) {
		
			var childname = childNameArr [childr];
...
//			$("#liEducationPlg_Dialog #selDlgEdPlgChldName").append( '<option value="'+childname+'">'+childname+'</option>' );
			$("#dlgli_EducationPlg_tab #txtFldDlgChildName").append( '<option value="'+childname+'">'+childname+'</option>' ); 
			$("#selDlgEdPlgChldName").append( '<option value="'+childname+'">'+childname+'</option>' );
			
		}
		

	}
	loadSlfSpsNameAftrDel();
}

function clearExistSearchChildName(){
	//$("#Childdtlstablbl").attr('checked', true).trigger('click');
	var invLen= $("#Investment_Dialog #selDlgInvNameOfChild option").length; 
	var cobLen= $("#Cob_Dialog #selDlgCOBChildName option").length;  
//	var inptinsLen= $("#listofLifeIns_Dialog #lipAssured option").length;  
	/*var lifeInsChildNamelen= $("#liEducationPlg_Dialog #selDlgEdPlgChldName option").length;*/
	...
	var DlglifeInsChildNamelen=  $("#dlgli_EducationPlg_tab #txtFldDlgChildName  option").length;  
	var cpfChildLen = $("#selDlgCPFNameOfChild option").length; 
	/*var EduPlgChildName = $("#liEducationPlg_Dialog #selDlgEdPlgChldName option").length; */ 
	var EduPlgChildName = $("#selDlgEdPlgChldName option").length; 
	var rowCount = $("#childParticularsTableNew tbody").find("tr").length;
...
//	 }
	 ...
	 for(var k=1;k<=EduPlgChildName;k++){
		 $("#selDlgEdPlgChldName option").remove(0);
	 }  
...
$(document).on("click", "#DNewChldPt", function () {
	...
	if(isOneRowSelected) {
		...
		if(deleteFlag){   
			...
			var EduPlgChldNameVal = $("#selDlgEdPlgChldName").val();
			...
			var LifeInsChldNameVal = $("#lipAssured").val();
		...
		if( alertFlagInv == 1 || alertFlagCob == 1  || alertFlagEduPlg == 1 || alertFlagDlgEduPlg == 1 || alertFlagLifeInsurance == 1){
			...
			$('#confirmationalertmsgdiv').on('shown.bs.modal', function() {  
				...
				$('#confirmationalertmsgdiv').find(".modal-footer").find("button:eq(0)").click(function (){   
					...
					addNewChildNameDynEle(delvalArr,'DEL');
					$('#childParticularsTableNew tbody').find('input[type=checkbox]').each(function(){
						if($(this).is(":checked")){
