...
$(document).ready(function(){
    ...
    html: true, 
	content: function() {
          return $('#CICoveragePopoverContent').html();
        }
});
});
...
function fipaInitPageClient() {
	...
	if (stafftype != STAFFTYPE_ADVISER) { // Amin
...
//	$("input:checkbox[name=txtFldClientChoice]:eq(5)").prop("disabled", true).prop("checked", false).val("N");  
	$("input:checkbox[name=txtFldClientChoice]:eq(4)").prop("disabled", false).prop("checked", true).val("Y");
...
//	showTooltipCls('otherliableicon', OTHERLIABLE_ICON); 
	...
	showTooltip('fipaInv_li','Investments, NAV and Dividend Details');//added by John 2204.1 23032022
	...
	showTooltip('OtherAssetArow','Add more other asset details');
	showTooltip('OtherAssetDrow','Delete the selected other asset(s) details');
...
//	clntreportdiv - Requirement to allow report for staff/adviser
	...
	switch (option) {
		...
		$(".autoAlt").each(function(){ 
			$(this).removeClass("alterColor");$(this).qtip('destroy',true);
		});
...
////	Need to check whether these default values required?
//	$("#dfSpsIdtype").val("NRIC");
//	$("#dfSelfIdtype").val("NRIC");
//	$("#dfSelfNationality").val("Singaporean");
//	$("#dfSpsNationality").val("Singaporean");
...
//	'fnahlthinsTbl',removed
	...
	var tblIds = ['childParticularsTableNew', 'childParticularsTable', 'OthRetPlgtbl', 'IncRetPlgtbl',
	  			...
	  			'fnaLINomineesTbl', 'liRetirementPlgtbl', 'cashOfBanksTable','srsTable','otherAssetTableNew' ];
...
//							console.log(key + "=" + value)
							if (key == "NO_CLIENT_PROFILE_LIST") {
								...
								$( "input:checkbox[name=txtFldClientChoice]:eq(4)").click();
...
//	$("#ArcProTab").attr('checked', true).trigger('click');
	...
	$('#clientsearch a[href="#profileArchive"]').tab('show');
	
	$("#txtNavClientDtls").show();
	/*$("#typesofappln_li").addClass("hidden");*/
	$('#childParticularsTableNew tbody tr').remove();
...
//	 old ids: 'childParticularsTable','deptParticularsTable',
	var tblIds = ['childParticularsTableNew','deptParticularsTableNew',
	              'finGoalsTable', 'wealthAccmltTable',
	              'OthRetPlgtbl', 'IncRetPlgtbl','IncAssRetPlgtbl', 'RDExptbl', 'RDInctbl','RDIncAsstbl', 'retCpfPayoutTable',
	              'existPolicyLHIns', 'existPolicyUtlip','fnaLINomineesTbl', 'liRetirementPlgtbl', 
	              'cpfMontlyContbTable','cpfAccDeductTable', 
	              'fnaPropOwnTblByCPF',
	              'othareaofconTbl',
	              'fnaAttachments', 
	              'fnaartuplanTbl', 'fnaartufundTbl','fnaswrepplanTbl', 'fnaSwrepFundTbl', 
	              'clntReasnsTable',
	              'cashOfBanksTable','srsTable','otherAssetTableNew','personalAssetTbl', 'businessAssetTbl', 'fnaVehiOwnTbl','tblInvDetsSummaryId'];
...
//				showLoader();
			...
			$('#CantCreateNewProfDialog').find(".modal-footer").find("button:eq(1)").click(function (){
				...
				$("input[name=radSrchSelectedId]").prop("checked",false)
			});
		});
							...
							if (key == "OPEN_CUST_DETS") {
								...
								for ( var cont in jsnData) {
									...
									if ($("#dfSpsRegmailaddrSame").val() == 'Y') {
										...
										if (($("input:checkbox[name=txtFldAnalyisFor]:eq(1)").val() == 'Y')) {
									...
									if (cont == "dfSpsRegmailaddrSame") {
										if (contvalue == 'Y') {
											if ($("#dfSpsMailaddrAsSelf").is(":not(:checked)")&& $("#dfSpsRegmailaddrSame").is(":not(:checked)")) {
												$("#dfSpsMailaddrAsSelf").prop("checked", true).val("Y");
											}
									...
									if (cont == "dfSelfName") {
										$("[name='txtFldClientName']").val( contvalue);
										$("span[id='txtFldClientName']").text(contvalue);
									}

									if (cont == "dfSelfNric") {
										$("span[id='txtFldClientNric']").text(contvalue);
										$("[name='txtFldClientNric']").val(contvalue);
									}
									
									if(cont == "fpmsCustid") {
										$("[name= 'custId']").val(contvalue)
									}

									if (cont == "dfSpsName") {
										$("[name='txtFldSpouseName']").val(contvalue);
									}
							...
							if (key == "ADVDCL_DETS") {
								...
								for ( var cont in jsnData) {
									...
									if (cont == "appClientChoice") {
										...
										if (contvalue.IS == "Y") {
											$("input:checkbox[name=txtFldClientChoice]:eq(4)").prop("checked", true).val("Y");
										} else {
											$("input:checkbox[name=txtFldClientChoice]:eq(4)").prop("checked", false).val("N");
...
//									}
							...
							if (key == "OTHERASSET_DETS") { 
								$.each(value, function(contkey, contvalue) {
									
									createOtherAssetRows();
									getOtherAssetRows(contvalue);
								});
...
//								
							...
							if (key == "CPF_RET_SUM_DETS") {
								jsonDataPopulate(value, tab);								

							}
							if (key == "CPF_CARESHIELD_DETS") {
								jsonDataPopulate(value, tab);								

							}
							if (key == "CPF_ELDERSHIELD_DETS") {
								jsonDataPopulate(value, tab);								

							}
							if (key == "CPF_MEDISHIELD_DETS") {
								jsonDataPopulate(value, tab);								

							}
							if (key == "CPF_DEPNDPROTECT_DETS") {
								jsonDataPopulate(value, tab);								

							}
							if (key == "CPF_HOUSEPROTECT_DETS") {
								jsonDataPopulate(value, tab);								

							}
							if (key == "SRS_WITHDRAW_DETS") {
								jsonDataPopulate(value, tab);	
							}
							if (key == "EXPD_DETS") {
								jsonDataPopulate(value, tab);  
							...
							if (key == "FPMS_POLICY_DETS") {
								...
								for ( var cont in jsnData) {
									...
									if (sublob == "ILP" || sublob == "UT") {
										...
										var cell1 = '<input type="text" readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor" style="display:inline;width:55px"/><input type="hidden" name="strFipaLifeInsId"/><input type="hidden" name="strFnaId"/><input type="hidden" name="strFPMSAppId"/><input type="hidden" name="strFIPARefId"/>'
												+ ((AppName == "FIPA" || AppName == "FIPA/FPMSNL") ? fipaSymbol
														: fpmsSymbol);
										/*
...
*/
										var cell2 = '<input type="text"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/>'+
										'<input type="text"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/>';
										var cell3 = '<input type="text"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/><input type="hidden"/>'+
										'<input type="text"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/>';
										...
										//utlipPolicyTbl.row.add( [ cell0, cell1, cell2, cell3, cell4, cell5, cell6, cell7, cell8, cell9,cell10,cell11,cell12 ]) .draw(false);
										  utlipPolicyTbl.row.add( [ cell0, cell1, cell2, cell3, cell4, cell5, cell6, cell7, cell8]) .draw(false);
										...
										if(AppName == "FIPA"|| AppName == "FIPA/FPMSNL"){
...
*/
										$lastRow.find("td:eq(2)").find('input:eq(0)').val(contvalue["strFPMSPolPlanName"]);
										
										$lastRow.find("td:eq(2)").find('input:eq(1)').val(prin);
										
										
										$lastRow.find("td:eq(3)").find('input:eq(1)').val(contvalue["strFPMSPolProposed"]);
										
										$lastRow.find("td:eq(3)").find('input:eq(0)').val(contvalue["strFPMSPolOwner"]);
										
										$lastRow.find("td:eq(3)").find('input:eq(2)').val(contvalue["strFPMSPolLifeAssured"]);
										
										$lastRow.find("td:eq(4)").find('input:eq(0)').val(contvalue["strFPMSPolPolNo"]);
										
										$lastRow.find("td:eq(4)").find('input:eq(1)').val(contvalue["strFPMSPolCustName"]);
										
										$lastRow.find("td:eq(5)").find('input:eq(0)').val(contvalue["strFPMSPolCoverages"]);

										$lastRow.find("td:eq(6)").find('input:eq(0)').val(contvalue["strFPMSPolStatus"]);
										
										$lastRow.find("td:eq(6)").find('input:eq(1)').val(contvalue["strFPMSPolEffDate"]); 
										
										$lastRow.find("td:eq(7)").find('input:eq(0)').val(contvalue["strFPMSPolSA"]);
										
										$lastRow.find("td:eq(7)").find('input:eq(0)').css("text-align","right");

										$lastRow.find("td:eq(8)").find('input:eq(0)').val(contvalue["strFPMSPolPremium"]);
										
										$lastRow.find("td:eq(8)").find('input:eq(1)').val(contvalue["strFPMSPolLOBMain"]+ "/" + sublob);
										...
										$lastRow.find("td:eq(8)").find('input:eq(0)').css("text-align","right");
											...
											var emptycell = "";
											var cell1 = '<input type="text" readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor" style="display:inline;width:55px"/><input type="hidden" name="strFipaLifeInsId"/><input type="hidden" name="strFnaId"/><input type="hidden" name="strFPMSAppId"/><input type="hidden" name="strFIPARefId"/>'
													+ ((AppName == "FIPA" || AppName == "FIPA/FPMSNL") ? fipaSymbol	: fpmsSymbol);
											var cell2 = '<input type="hidden"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/>'+
											'<input type="hidden"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/>'+
											'<div  name="" class="form-control editable form-tablecolor"   onmouseover="fipaTooltip(this);" readonly="readonly" style=" word-wrap: break-word;display: inline-block;"/></div>';
											/*var cell3 = '<input type="text"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/><input type="hidden"/>'+
											'<input type="text"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/>';*/
											var cell3 = '<input type="hidden"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/><input type="hidden"/>'+
											'<input type="hidden"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/>'+
											//'<input type="text" name="" class="dhtmlTableText ptrcursor"   onmouseover="fipaTooltip(this);" readonly="readonly"/>';
											'<div  name="" class="form-control editable form-tablecolor"   onmouseover="fipaTooltip(this);" readonly="readonly" style="word-wrap: break-word;display: inline-block;"/></div>';//Proposed
											var cell4 = '<input type="text"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/><input type="hidden"/>';
											//var cell5 = '<input type="text"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/>';
											//var cell5 ='<input type="hidden"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/>'+ 
											...
											var listval = contvalue["strFPMSPolCoverages"];
											
											var texttoshowcov = "";
											var arr = listval.toString().split(",");
											...
											if (cnt > 1) {
												texttoshowcov = "<div style='display:inline'> ..."
														+ texttoshowcov
														+ "<img data-image='Show' align='right' onmouseover='profileTooltipCov(this);' src='images/menuicons/icoDown.png' style='width:20px' ><div>";

											} else {
												texttoshowcov = contvalue["strFPMSPolCoverages"];
											}
											
											var cell5 = "<div data-image='Show'><div data-originaltext='"+ contvalue["strFPMSPolCoverages"]+ "'><ol>"+ texttoshowcov+ "</ol></div></div>";

											
											var cell6 = '<input type="text"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/>'+
											'<input type="hidden"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/>';
											/*var cell7 = '<input type="text"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/>';*/
											var cell7 = '<input type="hidden"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/>'+
											'<input type="text" name="" class="dhtmlTableText ptrcursor"   onmouseover="fipaTooltip(this);" readonly="readonly"/>';
											/*var cell8 = '<input type="text"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/>'+
											'<input type="text"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/>';*/
											var cell8 = '<input type="hidden"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/>'+
											'<input type="hidden"  readOnly="true" onmouseover="fipaTooltip(this);" class="dhtmlTableText ptrcursor"/>'+
											'<input type="text" name="" class="dhtmlTableText ptrcursor"   onmouseover="fipaTooltip(this);" readonly="readonly"/>';
											
											
											
											existPolicyLHIns.row.add( [ cell0, cell1, cell2, cell3,	cell4, cell5, cell6,cell7, cell8]).draw(false);
											...
											//$lastRow.find("td:eq(3)").find('input:eq(3)').val("Proposer: " + contvalue["strFPMSPolOwner"] + ", " + "Life Assured: " + contvalue["strFPMSPolLifeAssured"]); //Proposed
											
											//multiple value show (we want to set bold one value)
							                var addboldtagProposer = '<div style="font-weight: bold;">'+ "Proposer:" +'</div>';
							                var addboldtagAssured = '<div style="font-weight: bold;">'+"Life" +" "+ "Assured:" +'</div>';
											$lastRow.find("td:eq(3)").find('div').html(addboldtagProposer  + contvalue["strFPMSPolOwner"] + '<br>' + addboldtagAssured  + contvalue["strFPMSPolLifeAssured"] ); //Proposed
											
											$lastRow.find("td:eq(4)").find(
													'input:eq(0)').val(
											...
											var planNameDB = contvalue["strFPMSPolPlanName"];
											planNameDB = isEmpty(planNameDB)?"-NIL-":planNameDB;
											  var plantxt = '<div style="font-weight: bold;">'+ "Plan Name:" +'</div>';
								                var printxt = '<div style="font-weight: bold;">'+"Principal:" +'</div>';
											$lastRow.find("td:eq(2)").find('div').html(plantxt  +planNameDB  + '<br>' + printxt  + prin ); //Proposed
											lifePlanNamearr.push(""+planNameDB+"");
											$lastRow
											.find("td:eq(5)")
											.find('input:eq(0)')
											.val(
													contvalue["strFPMSPolCoverages"]);
										//	$lastRow.find("td:eq(5)").find('span').html(contvalue["strFPMSPolCoverages"]); //Proposed
											...
											$lastRow.find("td:eq(7)").find('input:eq(1)').val("$" + contvalue["strFPMSPolSA"]); //Proposed
											$lastRow.find("td:eq(7)").find('input:eq(1)').css("text-align","right"); //Proposed
											...
											$lastRow.find("td:eq(8)").find('input:eq(2)').val("$" + contvalue["strFPMSPolPremium"]); //Proposed
...
//										calSpsCpfMastMthContr();
							...
							//added by John 2204.1
							if (tab == "FIPA_INV_DETS"){
								fnPopulateInvDets(value);
							} 
...
//					$("#clearmsglifeData").find("button").prop("disabled",false);
				...
				if(FIPAMenu != null && FIPAMenu != '' && FIPAMenu != 'null' ){
					...
					$('a[href="'+FIPAMenu+'"]').trigger("click");
						if(FIPATab != null && FIPATab != '' && FIPATab != 'null' ){
						/* $("#hTxtTabName").val(""); */
						$('a[href="'+FIPATab+'"]').trigger("click");
					}
				}
...
//un used but we have to consider
								...
								for ( var cont in jsnData) {
									...
									if (sublob == "ILP" || sublob == "UT") {
										...
										*/
										...
										$lastRow.find("td:eq(2)").find('input:eq(0)').val(strFPMSPolPlanName);
									...
									/*	$lastRow
										...
										*/
										...
										$lastRow
										.find("td:eq(11)")
										.find('input:eq(0)').val(contvalue["strFPMSPolPremium"]);
										...
										.find("td:eq(12)")
										.find('input:eq(0)')
										.val(contvalue["strFPMSPolLOBMain"]+ "/" + sublob);*/
										...
										.find("td:eq(1)")
										.find('input:eq(0)')
										.val(contvalue["strFPMSPolApplnName"]);
										
										$lastRow
										.find("td:eq(1)")
										.find('input:eq(1)')
										.val(contvalue["strFipaLifeInsId"]);
										
										$lastRow
										.find("td:eq(1)")
										.find('input:eq(2)')
										.val(contvalue["strFnaId"]);
										
										$lastRow
										.find("td:eq(1)")
										.find('input:eq(3)')
										.val(contvalue["strFPMSPolAppId"]);
										
										$lastRow
										.find("td:eq(1)")
										.find('input:eq(4)')
										.val(contvalue["strFIPARefId"]);
										...
										.find("td:eq(3)")
										.find('input:eq(1)')
										.val(contvalue["strFPMSPolProposed"]);
										
										$lastRow
										.find("td:eq(3)")
										.find('input:eq(0)')
										.val(contvalue["strFPMSPolOwner"]);
										
										$lastRow
										.find("td:eq(3)")
										.find('input:eq(2)')
										.val(contvalue["strFPMSPolLifeAssured"]);
										
										
										$lastRow
										.find("td:eq(4)")
										.find('input:eq(0)')
										.val(contvalue["strFPMSPolPolNo"]);
										
										$lastRow
										.find("td:eq(4)")
										.find('input:eq(1)')
										.val(contvalue["strFPMSPolCustName"]);

										$lastRow
										.find("td:eq(2)")
										.find('input:eq(0)')
										.val(contvalue["strFPMSPolPlanName"]);
										
										$lastRow
										.find("td:eq(5)")
										.find('input:eq(0)')
										.val(contvalue["strFPMSPolCoverages"]);
										
										$lastRow
										.find("td:eq(6)")
										.find('input:eq(1)')
										.val(contvalue["strFPMSPolEffDate"]);
										
										$lastRow
										.find("td:eq(6)")
										.find('input:eq(0)')
										.val(contvalue["strFPMSPolStatus"]);
										
										$lastRow
										.find("td:eq(7)")
										.find('input:eq(0)')
										.val(contvalue["strFPMSPolSA"]);
										
										$lastRow
										.find("td:eq(7)")
										.find('input:eq(0)').css("text-align","right");
										
										$lastRow
										.find("td:eq(8)")
										.find('input:eq(0)')
										.val(contvalue["strFPMSPolPremium"]);
...
//move to common or date related func
...
// this is blunder, while onchange marital status, why its getting nationality autochange???	
//	$("#dfSelfNationality").val("Singaporean");
//	$("#dfSpsNationality").val("Singaporean");
...
//move this method to common js or ekyc.js since it is ekyc related items
...
$("#main_menu_pro_scrh").click(function(){
...
//	FIPAMenu = "";
	...
	$('#clientsearch a[href="#profilsearch"]').tab('show');
...
$("#btnClientParticularsPrint").click(function (){ 
	 
	var clientsectiondivId ='clientsection'; //clientsection     clientsectionone
	 
	var pl ="portrait";  // landscape     portrait
	var fileSave ="Client_Particulars_";
	var screentittle ="";  // Client Particulars  //screen title
	var screentittlefootername = "Client Particular";

	var jsonObject = 
	    [{"key":"Image","value":clientsectiondivId,"Title":screentittle,"TitleFooter":screentittlefootername,"header":"","pdfaddnewPage":""}	     
       ];
...
$("#btnspouseinformationPrint").click(function (){ 
	 
	 
	var spousesectiondivID = 'spousesection';
	 
	var pl ="portrait";  // landscape     portrait
	var fileSave ="Spouse_Particulars_";
	 
	var spousescreentittle =""; //Spouse Particulars
	 
	var screentittlefootername = "Spouse Particular";
	
	var jsonObject = 
	    [{"key":"Image","value":spousesectiondivID,"Title":spousescreentittle,"TitleFooter":screentittlefootername,"header":"","pdfaddnewPage":""},
