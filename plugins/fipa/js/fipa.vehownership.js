...
var cell2 = '<select  name="txtFldVehOwner" class="form-control editable form-tablecolor fontbold" onmouseover="fipaTooltip(this);" readonly="readonly" disabled="disabled"></select>';
 
var cell3 = '<input type="hidden" name="txtFldVehMktVal" class="form-control editable form-tablecolor fontbold" onmouseover="fipaTooltip(this);" readonly="readonly" />'+
'<span   onmouseover="fipaTooltip(this);"  style="width: 145px; word-wrap: break-word;display: inline-block; overflow: hidden;"/></span>';//Proposed; 

var cell4 = '<input type="hidden" name="txtFldVehLnVal" class="form-control editable form-tablecolor fontbold"   onmouseover="fipaTooltip(this);" readonly="readonly"  />'+
'<span  onmouseover="fipaTooltip(this);"  style="width: 145px; word-wrap: break-word;display: inline-block; overflow: hidden;"/></span>';//Proposed; 	
...
var cell5 = '<input type="hidden" name="txtFldVehiMonthInstal" class="form-control editable form-tablecolor fontbold"   onmouseover="fipaTooltip(this);"  />'+
...
'<span onmouseover="fipaTooltip(this);"  style="width: 145px; word-wrap: break-word;display: inline-block;"/></span>';//Proposed
var cell6 = '<select name="txtFldVehiSoldOnDeath" class="form-control editable form-tablecolor fontbold" readonly="readonly" disabled="disabled"></select>'+
...
var vehmkt=(isEmpty($("#txtFldDlgVehMktVal").val())) ? Number("0") : Number($("#txtFldDlgVehMktVal").val());
$lastRow.find("td:eq(3)").find('span').text("$" + vehmkt); //Proposed
...
var vehLnVal=(isEmpty($("#txtFldDlgVehLnVal").val())) ? Number("0") : Number($("#txtFldDlgVehLnVal").val());
$lastRow.find("td:eq(4)").find('span').text("$" + vehLnVal); //Proposed
...
var vehmonIns=(isEmpty($("#txtFldDlgVehMthlyInstal").val())) ? Number("0") : Number($("#txtFldDlgVehMthlyInstal").val());
var vehperd=(isEmpty($("#txtFldDlgVehPerd").val())) ? Number("0") : Number($("#txtFldDlgVehPerd").val());

//add vignesh on 24 06 2021
var addboldtagvehmonIns = '<span style="font-weight: bold;">'+"$" + vehmonIns + " monthly"+'</span>'; 
 
$lastRow.find("td:eq(5)").find('span').html(addboldtagvehmonIns + "," + '<br>' + "For " + vehperd + " years");

//$lastRow.find("td:eq(5)").find('span').html("$" + vehmonIns + " monthly" + "," + '<br>' + "For " + vehperd + " years"); //Proposed
...
if(dataset != null){
	...
	for(var data in dataset){
		...
		var monIns=(isEmpty(txtfldMontIns)) ? Number("0") : Number(txtfldMontIns);
		var vehPeriod=(isEmpty(txtfldVehiPeriod)) ? Number("0") : Number(txtfldVehiPeriod);
		var mkVal=(isEmpty(txtfldVehiMkvalue)) ? Number("0") : Number(txtfldVehiMkvalue);
		var loanVal=(isEmpty(txtfldVehiLoanval)) ? Number("0") : Number(txtfldVehiLoanval);
		
		//add vignesh on 24 06 2021
		var addboldtagmonIns = '<span style="font-weight: bold;">'+"$" + monIns + " monthly"+'</span>'; 
		 
		$lastRow.find("td:eq(5)").find('span').html(addboldtagmonIns + "," + '<br>' + "For " + vehPeriod + " years");
		
		//$lastRow.find("td:eq(5)").find('span').html("$" + monIns + " monthly" + "," + '<br>' + "For " + vehPeriod + " years"); //Proposed
		$lastRow.find("td:eq(3)").find('span').text("$" + mkVal); //Proposed
		$lastRow.find("td:eq(4)").find('span').text("$" + loanVal); //Proposed
...
$("#VehDRow").on("click",function(){ 
	datatableVehOwnDeleteRow('fnaVehiOwnTbl',fnaVehiOwnTbl,'Selvehicleownership','VehERow','VehDRow');  
/*//		DHTML CRUD ICONS
...
//	  $('#vehown_Dialog #txtFldDlgVehMode').val($lastRow.find("td:eq(0)").find('input:eq(0)').val());//instant save added line
	...
	//  $('#vehown_Dialog #txtFldDlgVehMthlyInstal').val($lastRow.find("td:eq(5)").find('input:eq(0)').val());
	 ...
	 // $row.find("td:eq(5)").find('input:eq(3)').val("$" + $("#txtFldDlgVehMthlyInstal").val() + "monthly" + ", " + "For" + $("#txtFldDlgVehPerd").val() + "years"); //propsed
	  ...
	  var mrktVal=(isEmpty($lastRow.find("td:eq(3)").find('input:eq(0)').val())) ? Number("0") : Number($lastRow.find("td:eq(3)").find('input:eq(0)').val());
	  $('#vehown_Dialog #txtFldDlgVehMktVal').val(mrktVal);
	  
	  var vehlnVal=(isEmpty($lastRow.find("td:eq(4)").find('input:eq(0)').val())) ? Number("0") : Number($lastRow.find("td:eq(4)").find('input:eq(0)').val());
	  $('#vehown_Dialog #txtFldDlgVehLnVal').val(vehlnVal);
	  
	  var vehMonInsl=(isEmpty($lastRow.find("td:eq(5)").find('input:eq(0)').val())) ? Number("0") : Number($lastRow.find("td:eq(5)").find('input:eq(0)').val());
	  $('#vehown_Dialog #txtFldDlgVehMthlyInstal').val(vehMonInsl);
		
	  var vehPeriod=(isEmpty($lastRow.find("td:eq(5)").find('input:eq(2)').val())) ? Number("0") : Number($lastRow.find("td:eq(5)").find('input:eq(2)').val());
	  $('#vehown_Dialog #txtFldDlgVehPerd').val(vehPeriod);
}
...
function vehfilldomval($RowId,$row){
	...
	var monIns=(isEmpty($("#txtFldDlgVehMthlyInstal").val())) ? Number("0") : Number($("#txtFldDlgVehMthlyInstal").val());
	var vehPer=(isEmpty($("#txtFldDlgVehPerd").val())) ? Number("0") : Number($("#txtFldDlgVehPerd").val());
	var vehMkt=(isEmpty($("#txtFldDlgVehMktVal").val())) ? Number("0") : Number($("#txtFldDlgVehMktVal").val());
	var VehLn=(isEmpty($("#txtFldDlgVehLnVal").val())) ? Number("0") : Number($("#txtFldDlgVehLnVal").val());
	//add vignesh on 24 06 2021
	var addboldtagmonIns = '<span style="font-weight: bold;">'+"$" + monIns + " monthly"+'</span>'; 
	 
	$row.find("td:eq(5)").find('span').html(addboldtagmonIns + "," + '<br>' + "For " + vehPer + " years");
	
	//$row.find("td:eq(5)").find('span').html("$" + monIns + " monthly" + "," + '<br>' + "For " + vehPer + " years"); //Proposed
	$row.find("td:eq(3)").find('span').text("$" + vehMkt); //Proposed
	$row.find("td:eq(4)").find('span').text("$" + VehLn); //Proposed
