...
$("#retirement_li").click(function(){
	reOrderIncAsstVisibleRows();
	});
	
$("#txtFldDlgORAgePaySts").on("change",function(){//aravindh

	if(!retStartAgeValidate($('#retSelfAge'),$("#txtFldDlgORAgePaySts")))return;//aravindh
});


$("#txtFldDlgIncAssAgePaySts").on("change",function(){//aravindh

	if(!retStartAgeValidate($('#retSelfAge'),$("#txtFldDlgIncAssAgePaySts")))return;//aravindh
});

function retStartAgeValidate(startAgeid,elmId){
	...
	if(!isEmpty(elmId.val())){
		if(elmId.val() <  startAge){
			applyToastrAlert("Age Payment Starts should greater than Coordinated retirement age ("+startAge+") ",elmId);
			elmId.val("");
			return false;
		}else
		if( elmId.val() > totAge){
			applyToastrAlert("Age Payment Starts should not be Greater than Projected life expectancy(Age)(Self) ("+totAge+")",elmId);
			elmId.val("");
			return false;
		}
	}
...
$lastRow.find("td:eq(9)").find('input:eq(0)').val($("#txtFldDlgORRemarks").val());*/
	...
	'<span   onmouseover="fipaTooltip(this);" style="width: 200px; word-wrap: break-word;display: inline-block;"/></span>';//Proposed; 
	...
	'<span  onmouseover="fipaTooltip(this);"  style="width: 185px; word-wrap: break-word;display: inline-block;"/></span>';//Proposed; 
	...
	$lastRow.find("td:eq(2)").find('input:eq(0)').val($("#txtFldDlgORtyofpay").val());
	/*var retPay=$("#txtFldDlgORtyofpay").val();
	if (retPay.indexOf(/\./g) == -1) {
		retPay=retPay.replace(/\./g,'_') ; 
		$lastRow.find("td:eq(2)").find('input:eq(0)').val(retPay);
		 alert("Chnaged val from Dialogue----"+retPay);
		}else{
			$lastRow.find("td:eq(2)").find('input:eq(0)').val($("#txtFldDlgORtyofpay").val());
		}*/
	...
	var annlExp=(isEmpty($("#txtFldDlgORAnlExp").val())) ? Number("0") : Number($("#txtFldDlgORAnlExp").val());
	var eslRate=(isEmpty($("#txtFldDlgOREslrate").val())) ? Number("0") : Number($("#txtFldDlgOREslrate").val());
	$lastRow.find("td:eq(3)").find('span').html("$" + annlExp + " Annually" + '<br>' + eslRate + " % Escalation"); //Proposed
	
	//$lastRow.find("td:eq(3)").find('span').html("$" + $("#txtFldDlgORAnlExp").val() + " Annually" + '<br>' + $("#txtFldDlgOREslrate").val() + " % Escalation"); //Proposed
	var oragebsed =  $("#selDlgORAgeBsOn > option").clone();
	...
	//$lastRow.find("td:eq(4)").find('span').html("Start @ " + $("#txtFldDlgORAgePaySts").val() + '<br>' + "End @ " + $("#txtFldDlgORAgePayends").val()); //Proposed
	...
	var ageStart=(isEmpty($("#txtFldDlgORAgePaySts").val())) ? Number("0") : Number($("#txtFldDlgORAgePaySts").val());
	var ageEnd=(isEmpty($("#txtFldDlgORAgePayends").val())) ? Number("0") : Number($("#txtFldDlgORAgePayends").val()); 
	//$lastRow.find("td:eq(4)").find('span').html("Start @ " + ageStart + '<br>' + "End @ " + ageEnd); //Proposed
	var freq=$("#selDlgORFreq").val();
	if(freq == 'SINGLE'){
		$lastRow.find("td:eq(4)").find('span').html("Start @ " + ageStart + '<br>' ); //Proposed
...
if(dataset != null){
	...
	for(var data in dataset){
		...
		var annlExp=(isEmpty(txtFldORAnlExp)) ? Number("0") : Number(txtFldORAnlExp);
		var escalRate=(isEmpty(txtFldOREslrate)) ? Number("0") : Number(txtFldOREslrate);
		//$lastRow.find("td:eq(3)").find('span').html("$" + txtFldORAnlExp + " Annually" + '<br>'  + txtFldOREslrate + " % Escalation"); //Proposed 
		$lastRow.find("td:eq(3)").find('span').html("$" + annlExp + " Annually" + '<br>'  + escalRate + " % Escalation"); //Proposed 
		var ageStart=(isEmpty(txtFldAgestart)) ? Number("0") : Number(txtFldAgestart);
		var ageEnd=(isEmpty(txtFldAgeend)) ? Number("0") : Number(txtFldAgeend);
		//$lastRow.find("td:eq(4)").find('span').html("Start @ " + txtFldAgestart + '<br>' + "End @ " + txtFldAgeend); //Proposed
		//$lastRow.find("td:eq(4)").find('span').html("Start @ " + ageStart + '<br>' + "End @ " + ageEnd); //Proposed
		var freq = txtFldFreq;
...
$("#DOthRet").on("click",function(){ 
	datatableOthPaymntDeleteRow('OthRetPlgtbl',OthRetPlgtbl,'SelOthRetPlgtbl','EOthRet','DOthRet');  
/*//			DHTML CRUD ICONS
...
//	  $('#OthRet_Dialog #txtFldDlgORMode').val($lastRow.find("td:eq(0)").find('input:eq(0)').val());//instant save added line
	  ...
	  var annlExp=(isEmpty($lastRow.find("td:eq(3)").find('input:eq(0)').val())) ? Number("0") : Number($lastRow.find("td:eq(3)").find('input:eq(0)').val());
	  $('#OthRet_Dialog #txtFldDlgORAnlExp').val(annlExp);
	 ...
	 // $('#OthRet_Dialog #txtFldDlgOREslrate').val($lastRow.find("td:eq(3)").find('input:eq(1)').val());
	  ...
	  var escRate=(isEmpty($lastRow.find("td:eq(3)").find('input:eq(1)').val())) ? Number("0") : Number($lastRow.find("td:eq(3)").find('input:eq(1)').val());
	  $('#OthRet_Dialog #txtFldDlgOREslrate').val(escRate);
	  
	  $('#OthRet_Dialog #selDlgORAgeBsOn').val($lastRow.find("td:eq(4)").find('select:eq(0)').val());
//	  loadAgeStartEnd($("#txtFldDlgORAgePaySts"),$("#selDlgORAgeBsOn")); 
	  var ageStart=(isEmpty($lastRow.find("td:eq(4)").find('input:eq(0)').val())) ? Number("0") : Number($lastRow.find("td:eq(4)").find('input:eq(0)').val());
	  $('#OthRet_Dialog #txtFldDlgORAgePaySts').val(ageStart);
...
//	  loadAgeEnd($("#txtFldDlgORAgePaySts"),$("#txtFldDlgORAgePayends")); 
	 // $('#OthRet_Dialog #txtFldDlgORAgePayends').val($lastRow.find("td:eq(4)").find('input:eq(1)').val());
	  var ageEnd=(isEmpty($lastRow.find("td:eq(4)").find('input:eq(1)').val())) ? Number("0") : Number($lastRow.find("td:eq(4)").find('input:eq(1)').val());
	  $('#OthRet_Dialog #txtFldDlgORAgePayends').val(ageEnd);
...
function othretfilldomval($RowId,$row){
	...
	if (retPay.indexOf(/\./g) == -1) {
		retPay=retPay.replace(/\./g,'_') ; 
		$row.find("td:eq(2)").find('input:eq(0)').val(retPay); 
		 alert("Chnaged val from Dialogue----"+retPay);
		}else{
	...
	//$row.find("td:eq(3)").find('span').html("$" + $("#txtFldDlgORAnlExp").val() + " Annually" + '<br>' + $("#txtFldDlgOREslrate").val() + " % Escalation"); //Proposed
	//$row.find("td:eq(4)").find('span').html("Start @ " + $("#txtFldDlgORAgePaySts").val() + '<br>' + "End @ " + $("#txtFldDlgORAgePayends").val()); //Proposed
	var annlExp=(isEmpty($("#txtFldDlgORAnlExp").val())) ? Number("0") : Number($("#txtFldDlgORAnlExp").val());
	var eslRate=(isEmpty($("#txtFldDlgOREslrate").val())) ? Number("0") : Number($("#txtFldDlgOREslrate").val());
	var ageStart=(isEmpty($("#txtFldDlgORAgePaySts").val())) ? Number("0") : Number($("#txtFldDlgORAgePaySts").val());
	var ageEnd=(isEmpty($("#txtFldDlgORAgePayends").val())) ? Number("0") : Number($("#txtFldDlgORAgePayends").val());
	
	$row.find("td:eq(3)").find('span').html("$" + annlExp + " Annually" + '<br>' + eslRate + " % Escalation"); //Proposed 
//	$row.find("td:eq(4)").find('span').html("Start @ " + ageStart + '<br>' + "End @ " + ageEnd); //Proposed
//	$row.find("input[type=text]").prop("readonly",true);//instant save added line
//	$row.find("select").prop("disabled",true);//instant save added line
	var freq=$("#selDlgORFreq").val();
	if(freq == 'SINGLE'){
		$row.find("td:eq(4)").find('span').html("Start @ " + ageStart + '<br>' ); //Proposed
	}else if(freq == 'REGULAR'){
		$row.find("td:eq(4)").find('span').html("Start @ " + ageStart + '<br>' + "End @ " + ageEnd); //Proposed
	}
...
'<input type="hidden" name="retIaDisbrsmntBy" class="form-control editable form-tablecolor"   onmouseover="fipaTooltip(this);" />'+
'<input type="hidden" name="retIaAnnlAmtOrYrs" class="form-control editable form-tablecolor"   onmouseover="fipaTooltip(this);" />'+
'<span   onmouseover="fipaTooltip(this);" style="width: 183px; word-wrap: break-word;display: inline-block;"/></span>';//Proposed
...
$lastRow.find("td:eq(0)").find('input[name=txtFldIncAssId]').val($("#txtFldDlgprojIncAssId").val());  
...
if((AddSts =="LIFE_RET")){
	...
	$lastRow.find("td:eq(0)").find('input[name=txtFldIncAssRefId]').val(rowRefID);
	
	//$lastRow.find("td:eq(0)").find('input:eq(2)').addClass("rowrefid");
	$lastRow.find("td:eq(0)").find('input[name=txtFldIncAssRefId]').addClass("rowrefid");
...
var amtOfInc=(isEmpty($("#txtFldDlgIncAssAmtofInc").val())) ? Number("0") : Number($("#txtFldDlgIncAssAmtofInc").val());
var roi=(isEmpty($("#txtFldDlgIncAssRoi").val())) ? Number("0") : Number($("#txtFldDlgIncAssRoi").val());
var eslrate=(isEmpty($("#txtFldDlgIncAssEslrate").val())) ? Number("0") : Number($("#txtFldDlgIncAssEslrate").val());

//var propsedHTML = "$" + amtOfInc + " " + $("#selDlgIncAssFreq").val() + '<br>' + eslrate + "% Escalation" + '<br>'+ roi +"% ROI";
var propsedHTML = "$" + amtOfInc + " " + $("#selDlgIncAssFreq").val() + '<br>'+ roi +"% ROI"+'<br>'+'<a class="btn btn-default addinfoicon"></a>';
$lastRow.find("td:eq(4)").find('span').html(propsedHTML); //Proposed
...
/*var agests=(isEmpty($("#txtFldDlgIncAssAgePaySts").val())) ? Number("0") : Number($("#txtFldDlgIncAssAgePaySts").val());
var ageeds=(isEmpty($("#txtFldDlgIncAssAgePayends").val())) ? Number("0") : Number($("#txtFldDlgIncAssAgePayends").val());*/
//$lastRow.find("td:eq(5)").find('span').html("Start @ " + agestarts + '<br>' + "End @ " + ageends); //Proposed
var freq=$("#selDlgIncAssFreq").val();
if(freq == 'SINGLE'){
	$lastRow.find("td:eq(5)").find('span').html("Start @ " + agestarts + '<br>'); //Proposed
}else if(freq == 'REGULAR'){
	$lastRow.find("td:eq(5)").find('span').html("Start @ " + agestarts + '<br>' + "End @ " + ageends); //Proposed
}
...
//	RDIncAsstbl.rows().remove().draw();
	...
	for(var data in dataset){
		...
		var retIaDisbrsmntBy;//Proposed
		var retIaAnnlAmtOrYrs;//Proposed
		...
		case "retIaDisbrsmntBy": 
			var value=(isEmpty(col)? ""  : col);
			$lastRow.find("td:eq(4)").find('input:eq(3)').val(value); 
			retIaDisbrsmntBy=value;//Proposed
			break;
			
		case "retIaAnnlAmtOrYrs": 
			var value=(isEmpty(col)? Number("0")  : col);
			$lastRow.find("td:eq(4)").find('input:eq(4)').val(value); 
			retIaAnnlAmtOrYrs=value;//Proposed
			break;
		...
		//$lastRow.find("td:eq(4)").find('span').html("$" + txtFldIncAssAmtofInc + " " + txtFldIncAssFreq + '<br>' + txtFldIncAssEslrate + "% Escalation" +'<br>'+ txtFldIncAssRoi +"% ROI" ); //Proposed
		//$lastRow.find("td:eq(5)").find('span').html("Start @ " + txtFldIncAssAgePaySts + '<br>' + "End @ " + txtFldIncAssAgePayends); //Proposed
		
		var agests=(isEmpty(txtFldIncAssAgePaySts)) ? Number("0") : Number(txtFldIncAssAgePaySts);
		var ageends=(isEmpty(txtFldIncAssAgePayends)) ? Number("0") : Number(txtFldIncAssAgePayends);
		//$lastRow.find("td:eq(5)").find('span').html("Start @ " + agests + '<br>' + "End @ " + ageends); //Proposed
		if(txtFldIncAssFreq == 'SINGLE'){
			$lastRow.find("td:eq(5)").find('span').html("Start @ " + agests + '<br>'); //Proposed
		}else if(txtFldIncAssFreq == 'REGULAR'){
			$lastRow.find("td:eq(5)").find('span').html("Start @ " + agests + '<br>' + "End @ " + ageends); //Proposed
		}
		var amtOfInc=(isEmpty(txtFldIncAssAmtofInc)) ? Number("0") : Number(txtFldIncAssAmtofInc);
		var eslrate=(isEmpty(txtFldIncAssEslrate)) ? Number("0") : Number(txtFldIncAssEslrate);
		var assroi=(isEmpty(txtFldIncAssRoi)) ? Number("0") : Number(txtFldIncAssRoi);
		
		var retDesc = $lastRow.find("td:eq(2)").find('input:eq(0)').val();
		var strClssfy = $lastRow.find("td:eq(3)").find('select:eq(0)').val();
		
		if(assroi != 0){
			
			if(strClssfy == 'Investment'){
				
				if(retIaDisbrsmntBy == 'YEARS'){
			
					var period = retIaAnnlAmtOrYrs;
					var annAmt = (-fnCalcPMT(assroi, period, amtOfInc, 0, "BEGINNING", "ANNUAL")).round(2);
				
				}else if(retIaDisbrsmntBy == 'AMOUNT'){
					
					var annAmt = retIaAnnlAmtOrYrs;
					var period = fnGetNPERVal(assroi, -annAmt, amtOfInc, 0, true);
				
				}//end of if
				
				$lastRow.find("td:eq(4)").find('span').html("$" + annAmt + " " + txtFldIncAssFreq +'<br>'+ assroi +"% ROI"+'<br>'+'<a class="btn btn-default addinfoicon"></a>' ); //Proposed
				
				$lastRow.find("td:eq(4)").find('span').find('a').on("click",function(){
					fnShowRetAnnualAmtDets(amtOfInc,annAmt,period,assroi,retDesc,retIaDisbrsmntBy); 
				});
				
			}else{
				
				$lastRow.find("td:eq(4)").find('span').html("$" + amtOfInc + " " + txtFldIncAssFreq + '<br>' + eslrate + "% Escalation" +'<br>'+ assroi +"% ROI" ); //Proposed
			...
			if(strClssfy == 'Investment'){
				...
				$lastRow.find("td:eq(4)").find('span').html("$" + amtOfInc + " " + txtFldIncAssFreq  +'<br>'+ "-"+'<br>'+'<a class="btn btn-default addinfoicon"></a>' ); //Proposed
				...
				$lastRow.find("td:eq(4)").find('span').html("$" + amtOfInc + " " + txtFldIncAssFreq + '<br>' + eslrate + "% Escalation" +'<br>'+ "-" ); //Proposed
...
$("#DIncAssRet").on("click",function(){ 
	datatableIncAsstDeleteRow("IncAssRetPlgtbl",IncAssRetPlgtbl,'SelIncAssRetPlgtbl','EIncAssRet','DIncAssRet');  
...
//	  $('#IncAssRet_Dialog #txtFldDlgIncAssMode').val($lastRow.find("td:eq(0)").find('input:eq(0)').val());//instant save added line
	...
	//  $('#IncAssRet_Dialog #txtFldDlgIncAssAgePayends').val($lastRow.find("td:eq(5)").find('input:eq(1)').val());
	  ...
	  var amtInc=(isEmpty($lastRow.find("td:eq(4)").find('input:eq(0)').val())) ? Number("0") : Number($lastRow.find("td:eq(4)").find('input:eq(0)').val());
	  $('#IncAssRet_Dialog #txtFldDlgIncAssAmtofInc').val(amtInc);
	  
	  var eslrate=(isEmpty($lastRow.find("td:eq(4)").find('input:eq(1)').val())) ? Number("0") : Number($lastRow.find("td:eq(4)").find('input:eq(1)').val());
	  $('#IncAssRet_Dialog #txtFldDlgIncAssEslrate').val(eslrate);
	  
	  var roi=(isEmpty($lastRow.find("td:eq(4)").find('input:eq(2)').val())) ? Number("0") : Number($lastRow.find("td:eq(4)").find('input:eq(2)').val());
	  $('#IncAssRet_Dialog #txtFldDlgIncAssRoi').val(roi);
	  
	  var paysts=(isEmpty($lastRow.find("td:eq(5)").find('input:eq(0)').val())) ? Number("0") : Number($lastRow.find("td:eq(5)").find('input:eq(0)').val());
	  $('#IncAssRet_Dialog #txtFldDlgIncAssAgePaySts').val(paysts);
	  
	  var payends=(isEmpty($lastRow.find("td:eq(5)").find('input:eq(1)').val())) ? Number("0") : Number($lastRow.find("td:eq(5)").find('input:eq(1)').val());
	  $('#IncAssRet_Dialog #txtFldDlgIncAssAgePayends').val(payends);
}
...
//	$row.find("td:eq(3)").find('input:eq(0)').val($("#txtFldDlgIncAssClsfy").val()); 
	...
	//$row.find("td:eq(4)").find('span').html("$" + $("#txtFldDlgIncAssAmtofInc").val() + " " + $("#selDlgIncAssFreq").val() + '<br>' + $("#txtFldDlgIncAssEslrate").val() + "% Escalation" +'<br>'+ $("#txtFldDlgIncAssRoi").val() +"% ROI" ); //Proposed
	//$row.find("td:eq(5)").find('span').html("Start @ " + $("#txtFldDlgIncAssAgePaySts").val() + '<br>' + "End @ " + $("#txtFldDlgIncAssAgePayends").val()); //Proposed
	
	
	var amtInc=(isEmpty($("#txtFldDlgIncAssAmtofInc").val())) ? Number("0") : Number($("#txtFldDlgIncAssAmtofInc").val());
	var eslRate=(isEmpty($("#txtFldDlgIncAssEslrate").val())) ? Number("0") : Number($("#txtFldDlgIncAssEslrate").val());
	var roi=(isEmpty($("#txtFldDlgIncAssRoi").val())) ? Number("0") : Number($("#txtFldDlgIncAssRoi").val());
	/*var ageStart=(isEmpty($("#txtFldDlgIncAssAgePaySts").val())) ? Number("0") : Number($("#txtFldDlgIncAssAgePaySts").val());
	var ageEnd=(isEmpty($("#txtFldDlgIncAssAgePayends").val())) ? Number("0") : Number($("#txtFldDlgIncAssAgePayends").val());*/
	...
	$row.find("td:eq(4)").find('span').html("$" + amtInc + " " + $("#selDlgIncAssFreq").val() + '<br>' + eslRate + "% Escalation" +'<br>'+ roi +"% ROI" ); //Proposed
	//$row.find("td:eq(5)").find('span').html("Start @ " + agestarts + '<br>' + "End @ " + ageends); //Proposed
	$row.find("td:eq(5)").find('input:eq(0)').val(agestarts);  
	$row.find("td:eq(5)").find('input:eq(1)').val(ageends);  
	//	$row.find("input[type=text]").prop("readonly",true);//instant save added line
	//$row.find("select").prop("disabled",true);//instant save added line
	var freq=$("#selDlgIncAssFreq").val();
	if(freq == 'SINGLE'){
		$row.find("td:eq(5)").find('span').html("Start @ " + agestarts + '<br>'); //Proposed
	}else if(freq == 'REGULAR'){
		$row.find("td:eq(5)").find('span').html("Start @ " + agestarts + '<br>' + "End @ " + ageends); //Proposed
	}
	...
	if(syncOn == "INV"){
		$("#fnaInvestmentTbl tbody").find("tr[rowrefid="+$rowref+"]").each(function(){ 
			applyToastrAlert("Retirement data will be reflected back to Investment Screen !"); 
			  $(this).find("td:eq(7)").find('input[name="txtFldInvAnnlDivid"]').val(amountofincome);
			  $(this).find("td:eq(2)").find('select[name="selInvOwner"]').val(ownership); 
			  //Cpf table change 
			...
			applyToastrAlert("Retirement data will be reflected back to Cash At Bank Screen !"); 
			  if(ownership == "Self"){	
				 // $(this).find("td:eq(2)").find('option:eq(1)').prop("selected","selected");
				  $(this).find("td:eq(2)").find('select[name=selCOBOwnershipType]').find('option:eq(1)').prop("selected","selected");
			  }else if(ownership == "Spouse"){	
				 // $(this).find("td:eq(2)").find('option:eq(2)').prop("selected","selected");
				  $(this).find("td:eq(2)").find('select[name=selCOBOwnershipType]').find('option:eq(2)').prop("selected","selected");
			  }else{	
				 // $(this).find("td:eq(2)").find('option:eq(0)').prop("selected","selected");
				  $(this).find("td:eq(2)").find('select[name=selCOBOwnershipType]').find('option:eq(0)').prop("selected","selected");
			  }
			 // $(this).find("td:eq(9)").find('input:eq(0)').val(amountofincome);
			  $(this).find("td:eq(4)").find('input[name="txtFldCOBCurBalance"]').val(amountofincome);  
			//  $(this).find("td:eq(5)").find('select:eq(0)').val(ownership);  
			  $(this).find("td:eq(2)").find('select[name=selCOBOwnershipType]').val(ownership);
		});
	}
...
var accr1 = document.querySelectorAll(".panel-headings");
var cont1 = document.querySelectorAll(".panel-collapses");
...
cont1.forEach(x => {
if(x.classList.contains("in")){
	li2.innerHTML = x.previousElementSibling.querySelector("a").innerText;
