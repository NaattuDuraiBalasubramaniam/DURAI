...
//		var hori=$("#cvplnleftpaneltbl tbody").find("tr:eq(0)").find("td:last").find("div.hiddens").find("input:eq(0)").prop("id").split("B");   
		...
		$("#cvplnleftpaneltbl tbody").find("tr:eq(0)").find("td:eq("+total_cell+")").find("div.hiddens").find("input:eq(9)").val("20");//<!-- Prct used-->
	...
	$("#cvplnleftpaneltbl tbody").find("tr:eq(1)").find("td:eq("+total_cell+")").find('input:eq(0)').each(function(){
		showTooltipByElem(this,$(this).val());
	});
	
	$("#cvplnleftpaneltbl tbody").find("tr:eq(7)").find("td:eq("+total_cell+")").find('input:eq(0)').each(function(){
		if(isEmpty($(this).val())){
			changeNRExpDate($(this));
		}
//		To calculate the Total Premium for each plans
		calcTotSAPremPlandetails();
	});
...
function genRiderfn(curElm,validateprevdata,dataset){
	...
	if(isEmpty(totalRiders) || totalRiders == "0" || totalRiders == 0) {
		totalRiders = "1";
	}
...
//	hiddens
	...
	$("#cvplnleftpaneltbl tbody").find("tr:eq(1)").find("td:eq("+index+")").find('input:eq(0)').each(function(){
		showTooltipByElem(this,$(this).val());
	});
	
	$("#cvplnleftpaneltbl tbody").find("tr:eq(7)").find("td:eq("+index+")").find('input:eq(0)').each(function(){
		if(isEmpty($(this).val())){
			changeNRExpDate($(this));
		}
//		To calculate the Total Premium for each plans
...
function showSelectedRet($reference){
	...
	var liRetirementPlglen = liRetirementPlgtbl.rows().count();
	//alert("count----"+liRetirementPlglen)
	if(liRetirementPlglen > 0){ 
		$("#liRetirementPlgtbl tbody tr").each(function(){
//			var CurRef = $(this).find("td:first").find('input:eq(2)').val();  
			var CurRef = $(this).find("td:first").find('input[name="txtFldMvRetRefId"]').val(); 
			$(this).hide(); 
			//alert(CurRef + "=="+ $reference);
			if(CurRef == $reference){ 
				$(this).show(); 
			}
...
function showSelectedEdu($reference){
	...
	var liEducationlen = liEduPayoutstbl.rows().count();
	//alert("liEducationlen----"+liEducationlen);
	if(liEducationlen > 0){
		$("#liEduPayoutstbl tbody tr").each(function(){
			var CurRef = $(this).find("td:first").find('input:eq(3)').val();  
			$(this).hide(); 
			//alert($reference+"---------"+CurRef);
			if(CurRef == $reference){
				$(this).show();  
			}
		});
	...
	reOrderVisibleRows("liEduPayoutstbl"); 
...
function ForCoverageClone(cur){
	...
	var $reference   = $("#cvplnleftpaneltbl tbody tr:eq(0) td:eq("+$tdpos+")").find("[name=txtFldliplnId]").val();//pkid no this is plan ref id
...
//		$(this).removeClass("selectedd");
	...
	/*$("#plancoveragetbl tbody tr:eq(0)").find("td:eq(1)").each(function(){ 
		$(this).find("input:eq(2)").val($reference);
	});*/
	/*$("#plancoveragetbl tbody tr").find("td").find("div").find('input[name="hCoveragePKId"]').each(function(){ 
		$(this).val($reference);
	});*/
...
/*	$("#plancoveragetbl tbody tr:eq(1)").find("td").each(function(){ 
	 ...
	 $("#tabheadRetirement").addClass("theaddisplayshowretirement");
	 $("#divRetirementPlgtbl").addClass("theaddisplayshowretirement");
	 $("#tabheadEducation").addClass("theaddisplayshowretirement");
	 $("#divEducationPlgtbl").addClass("theaddisplayshowretirement");
...
//		 $("#longTermdisBenefit").val("1");
		 
		 $("#covSA").val($SA);
		 
		 $("#maaDeath").val($SA);
		 $("#disBenefit").val($SA);
		 $("#longTermdisBenefit").val($SA);
		 $("#disTPD").val($SA);
		 $("#maaTPD").val($SA);
		 $("#maaCriIllAdv").val($SA);
		 $("#maaCriIllErly").val($SA);
		 $("#maaPA").val($SA);
		...
		$('#mulTPD').on('change', function (e) {
		   var boosterFactor = $("#mulTPD").val();
		   var disBenefit = $("#disTPD").val();
		   var cal=boosterFactor *disBenefit;
		   $("#maaTPD").val(cal); 
		});
		
		$('#disTPD').on('change', function (e) {
		   var boosterFactor = $("#mulTPD").val();
		   var disBenefit = $("#disTPD").val();
		   var cal=boosterFactor *disBenefit;
		   $("#maaTPD").val(cal); 
...
function toCalMAA(multiBootElm,MAAElm){
	...
	curelm = isEmpty(curelm) ? '1' :curelm;
	
	var curSumAssured = $("#"+MAAElm).val() ; 
	curSumAssured = isEmpty(curSumAssured) ? Number(0): Number(curSumAssured);
...
//	added
	...
	selectedCover = $hcov.split(",");
	 ...
	 if(strMode == INS_MODE || strMode == UPD_MODE){
		...
		$("#plancoveragetbl tbody tr:visible").each(function(colindex){ 
				//if(colindex > 0){
				...
				//	var isVisible = $(this).is(":visible");
					...
					//	if(isVisible){
							...
							/*var strTypeofCov 		= $(this).find("input").val();
							var strbooster  		= $("#plancoveragetbl tbody tr").find("td").find('input[name="mulBootDeath"]').val();
							var strmaaAmtAsd  		= $("#plancoveragetbl tbody tr").find("td").find('input[name="maaDeath"]').val();
							var strmaaExpPerd 		= $("#plancoveragetbl tbody tr").find("td").find('input[name="expDeath"]').val();
							var strnoOfYrs 			= $("#plancoveragetbl tbody tr").find("td").find('input[name="yrPayDeath"]').val();
							var strDsbltyBenf 		= $("#plancoveragetbl tbody tr").find("td").find('input[name="disBenefit"]').val();
							var stryrsOfDsbltyPay	= $("#plancoveragetbl tbody tr").find("td").find('input[name="disPayout"]').val();
							var strmaxPayoutAge		= $("#plancoveragetbl tbody tr").find("td").find('input[name="maxPayout"]').val();
							var strtypesOfHS  		= $("#plancoveragetbl tbody tr").find("td").find('input[name="typHSBenefit"]').val();
							var strdeductible 		= $("#plancoveragetbl tbody tr").find("td").find('input[name="dedHSBenefit"]').val();
							var strcoInsurance 		= $("#plancoveragetbl tbody tr").find("td").find('input[name="insHSBenefit"]').val();
							var strdescription		= $("#plancoveragetbl tbody tr").find("td").find('input[name="desHSBenefit"]').val();
							var strremarks 			= $("#plancoveragetbl tbody tr").find("td").find('input[name="remarks"]').val();
							var strCovPKID 			= $("#plancoveragetbl tbody tr").find("td").find('input[name="hCoveragePKId"]').val();*/
							
							//var deathbenf 		= $("#divmulBootDeathDiv").find('input[name="hCoverageType"]').val(); 
							var deathbenf 		=  $("#hdeathbenf").val();
							if(deathbenf == "Death_Benefit"){
								/*var strTypeofCov 		= $(this).find("input").val();
								var strbooster  		= $("#plancoveragetbl tbody tr").find("td").find('input[name="mulBootDeath"]').val();
								var strmaaAmtAsd  		= $("#plancoveragetbl tbody tr").find("td").find('input[name="maaDeath"]').val();
								var strmaaExpPerd 		= $("#plancoveragetbl tbody tr").find("td").find('input[name="expDeath"]').val();
								...
								var strTypeofCov 		= $("#hdeathbenf").val();
								var strbooster  		= $("#mulBootDeath").val();
								var strmaaAmtAsd  		= $("#maaDeath").val();
								var strmaaExpPerd 		= $("#expDeath").val();
								var strnoOfYrs 			= $("#yrPayDeath").val();
								var strDsbltyBenf 		= "";
								var stryrsOfDsbltyPay	= "";
								var strmaxPayoutAge		= "";
								var strtypesOfHS  		= "";
								...
								var strcoInsurance 		= "";
								var strdescription		= "";
								var strremarks 			= $("#remarks1").val();
								var strCovPKID 			= $("#hdeathbenfPK").val();
								var strlongTermdisADL	= "";
								...
								if(strTypeofCov != undefined && strTypeofCov != "undefined" && 
										...
										if( $lastRow == null ){ 
											...
											var cell17 ='<input type="text" name="txtFldliRemarks" class="form-control editable"  readonly="true"/>'+
											'<input type="hidden" name="txtFldliCovCrtdBy"/><input type="hidden" name="txtFldliCovCrtdDate"/>'; 
											var cell18 ='<input type="text" name="txtFldADL" class="form-control editable"  readonly="true"/>'
											planCoveragestbl.row.add( [cell0,cell1,cell2,cell3,cell4,cell5,cell6,cell7,cell8,cell9,cell10,cell11,cell12,cell13,cell14,cell15,cell16,cell17,cell18] ).draw( false );
											...
											$lastRow.find("td:eq(16)").find('input:eq(0)').val(strdescription);    
											$lastRow.find("td:eq(17)").find('input:eq(0)').val(strremarks);    
											$lastRow.find("td:eq(18)").find('input:eq(0)').val(strlongTermdisADL);
		
								
							}
							}
							var disbenf 		= $("#hdisability").val();
							//var disbenf 		= $("#divdisBenefitDiv").find('input[name="hCoverageType"]').val(); 
							if(disbenf == "Disability"){
								...
								var strTypeofCov 		= $("#hdisability").val();
								var strbooster  		= "";
								var strmaaAmtAsd  		= "";
								var strmaaExpPerd 		= "";
								var strnoOfYrs 			= "";
								var strDsbltyBenf 		= $("#disBenefit").val();
								var stryrsOfDsbltyPay	= $("#disPayout").val();
								var strmaxPayoutAge		= $("#maxPayout").val();
								var strtypesOfHS  		= "";
								var strdeductible 		= "";
								var strcoInsurance 		= "";
								var strdescription		= "";
								var strremarks 			= $("#remarks2").val();
								var strCovPKID 			= $("#hdisablebenfPK").val();
								var strlongTermdisADL	= "";
								if(strTypeofCov != undefined && strTypeofCov != "undefined" && 
										strTypeofCov != "Retirement_Planning" && strTypeofCov  != "Education_Planning"){ 
							...
							var disbenf 		= $("#hlongtermdisability").val();
							if(disbenf == "LongTermDisability"){
								
								var strTypeofCov 		= $("#hlongtermdisability").val();
								var strbooster  		= "";
								var strmaaAmtAsd  		= "";
								var strmaaExpPerd 		= "";
								var strnoOfYrs 			= "";
								var strDsbltyBenf 		= $("#longTermdisBenefit").val();
								var stryrsOfDsbltyPay	= $("#longTermdisPayout").val();
								var strmaxPayoutAge		= $("#longTermmaxPayout").val();
								var strlongTermdisADL	= $("#longTermdisADL").val();
								var strtypesOfHS  		= "";
								var strdeductible 		= "";
								var strcoInsurance 		= "";
								var strdescription		= "";
								var strremarks 			= $("#remarks13").val();
								var strCovPKID 			= $("#hlongTermdisablebenfPK").val();
								if(strTypeofCov != undefined && strTypeofCov != "undefined" && 
										strTypeofCov != "Retirement_Planning" && strTypeofCov  != "Education_Planning"){ 
							...
							var mulTPD 		= $("#hTpd").val();
							//var mulTPD 		= $("#divmulTPDDiv").find('input[name="hCoverageType"]').val(); 
							if(mulTPD == "TPD"){
								/*var strTypeofCov 		= $(this).find("input").val();
								var strbooster  		= $("#plancoveragetbl tbody tr").find("td").find('input[name="mulTPD"]').val();
								var strmaaAmtAsd  		= $("#plancoveragetbl tbody tr").find("td").find('input[name="maaTPD"]').val();
								var strmaaExpPerd 		= $("#plancoveragetbl tbody tr").find("td").find('input[name="expTPD"]').val();
								var strnoOfYrs 			= "";
								var strDsbltyBenf 		= $("#plancoveragetbl tbody tr").find("td").find('input[name="maaDeath"]').val();
								var stryrsOfDsbltyPay	= $("#plancoveragetbl tbody tr").find("td").find('input[name="disPayoutTPD"]').val();
								var strmaxPayoutAge		= $("#plancoveragetbl tbody tr").find("td").find('input[name="maxPayoutTPD"]').val();
								var strtypesOfHS  		= "";
								var strdeductible 		= "";
								var strcoInsurance 		= "";
								var strdescription		= "";
								...
								var strTypeofCov 		= $("#hTpd").val();
								var strbooster  		= $("#mulTPD").val();
								var strmaaAmtAsd  		= $("#maaTPD").val();
								var strmaaExpPerd 		= $("#expTPD").val();
								var strnoOfYrs 			= "";
								var strDsbltyBenf 		= $("#disTPD").val();
								var stryrsOfDsbltyPay	= $("#disPayoutTPD").val();
								var strmaxPayoutAge		= $("#maxPayoutTPD").val();
								var strtypesOfHS  		= "";
								var strdeductible 		= "";
								var strcoInsurance 		= "";
								var strdescription		= "";
								var strremarks 			= $("#remarks3").val();
								var strCovPKID 			= $("#hTPDbenfPK").val();
								var strlongTermdisADL	= "";
								if(strTypeofCov != undefined && strTypeofCov != "undefined" && 
										strTypeofCov != "Retirement_Planning" && strTypeofCov  != "Education_Planning"){ 
							...
							var criticalAdv 		= $("#hCIAdvanced").val();
							//var criticalAdv 		= $("#divmulCriIllAdvDiv").find('input[name="hCoverageType"]').val(); 
							if(criticalAdv == "CI-Advanced"){
								/*var strTypeofCov 		= $(this).find("input").val();
								var strbooster  		= $("#plancoveragetbl tbody tr").find("td").find('input[name="mulCriIllAdv"]').val();
								var strmaaAmtAsd  		= $("#plancoveragetbl tbody tr").find("td").find('input[name="maaCriIllAdv"]').val();
								var strmaaExpPerd 		= $("#plancoveragetbl tbody tr").find("td").find('input[name="expCriIllAdv"]').val();
								var strnoOfYrs 			= "";
								var strDsbltyBenf 		= $("#plancoveragetbl tbody tr").find("td").find('input[name="disCriIllAdv"]').val();
								var stryrsOfDsbltyPay	= $("#plancoveragetbl tbody tr").find("td").find('input[name="disPayoutCriIllAdv"]').val();
								var strmaxPayoutAge		= $("#plancoveragetbl tbody tr").find("td").find('input[name="maxPayoutCriIllAdv"]').val();
								var strtypesOfHS  		= "";
								var strdeductible 		= "";
								var strcoInsurance 		= "";
								var strdescription		= "";
								...
								var strTypeofCov 		= $("#hCIAdvanced").val();
								var strbooster  		= $("#mulCriIllAdv").val();
								var strmaaAmtAsd  		= $("#maaCriIllAdv").val();
								var strmaaExpPerd 		= $("#expCriIllAdv").val();
								var strnoOfYrs 			= "";
								var strDsbltyBenf 		= $("#disCriIllAdv").val();
								var stryrsOfDsbltyPay	= $("#disPayoutCriIllAdv").val();
								var strmaxPayoutAge		= $("#maxPayoutCriIllAdv").val();
								var strtypesOfHS  		= "";
								var strdeductible 		= "";
								var strcoInsurance 		= "";
								var strdescription		= "";
								var strremarks 			= $("#remarks4").val();
								var strCovPKID 			= $("#hciadvancedbenfPK").val();
								var strlongTermdisADL	= "";
								if(strTypeofCov != undefined && strTypeofCov != "undefined" && 
										strTypeofCov != "Retirement_Planning" && strTypeofCov  != "Education_Planning"){ 
							...
							var criticalErly 		= $("#hCIEarly").val();
							//var criticalErly 		= $("#divmulCriIllErlyDiv").find('input[name="hCoverageType"]').val(); 
							if(criticalErly == "CI-Early"){
								/*var strTypeofCov 		= $(this).find("input").val();
								var strbooster  		= $("#plancoveragetbl tbody tr").find("td").find('input[name="mulCriIllErly"]').val();
								var strmaaAmtAsd  		= $("#plancoveragetbl tbody tr").find("td").find('input[name="maaCriIllErly"]').val();
								var strmaaExpPerd 		= $("#plancoveragetbl tbody tr").find("td").find('input[name="expCriIllErly"]').val();
								var strnoOfYrs 			= $("#plancoveragetbl tbody tr").find("td").find('input[name="yrPayCriIllErly"]').val();
								var strDsbltyBenf 		= "";
								var stryrsOfDsbltyPay	= "";
								var strmaxPayoutAge		= "";
								var strtypesOfHS  		= "";
								...
								var strTypeofCov 		= $("#hCIEarly").val();
								var strbooster  		= $("#mulCriIllErly").val();
								var strmaaAmtAsd  		= $("#maaCriIllErly").val();
								var strmaaExpPerd 		= $("#expCriIllErly").val();
								var strnoOfYrs 			= $("#yrPayCriIllErly").val();
								var strDsbltyBenf 		= "";
								var stryrsOfDsbltyPay	= "";
								var strmaxPayoutAge		= "";
								var strtypesOfHS  		= "";
								...
								var strcoInsurance 		= "";
								var strdescription		= "";
								var strremarks 			= $("#remarks5").val();
								var strCovPKID 			= $("#hciearlybenfPK").val();
								var strlongTermdisADL	= "";
								if(strTypeofCov != undefined && strTypeofCov != "undefined" && 
										strTypeofCov != "Retirement_Planning" && strTypeofCov  != "Education_Planning"){ 
							...
							var pa 		= $("#hPA").val();
							//var pa 		= $("#divmulPADiv").find('input[name="hCoverageType"]').val(); 
							if(pa == "PA"){
								/*var strTypeofCov 		= $(this).find("input").val();
								var strbooster  		= $("#plancoveragetbl tbody tr").find("td").find('input[name="mulPA"]').val();
								var strmaaAmtAsd  		= $("#plancoveragetbl tbody tr").find("td").find('input[name="maaPA"]').val();
								var strmaaExpPerd 		= $("#plancoveragetbl tbody tr").find("td").find('input[name="expPA"]').val();
								var strnoOfYrs 			= $("#plancoveragetbl tbody tr").find("td").find('input[name="yrPayPA"]').val();
								var strDsbltyBenf 		= "";
								var stryrsOfDsbltyPay	= "";
								var strmaxPayoutAge		= "";
								var strtypesOfHS  		= "";
								...
								var strTypeofCov 		= $("#hPA").val();
								var strbooster  		= "";
								var strmaaAmtAsd  		= ""
								var strmaaExpPerd 		= $("#expPA").val();
								var strnoOfYrs 			= $("#yrPayPA").val();
								var strDsbltyBenf 		= $("#maaPA").val();
								var stryrsOfDsbltyPay	= "";
								var strmaxPayoutAge		= "";
								var strtypesOfHS  		= "";
								var strdeductible 		= "";
								var strcoInsurance 		= "";
								var strdescription		= "";
								var strremarks 			= $("#remarks6").val();
								var strCovPKID 			= $("#hpabenfPK").val();
								var strlongTermdisADL	= "";
								if(strTypeofCov != undefined && strTypeofCov != "undefined" && 
										strTypeofCov != "Retirement_Planning" && strTypeofCov  != "Education_Planning"){ 
							...
							var hospital 		= $("#hHSP").val();
							//var hospital 		= $("#divmulHSPDiv").find('input[name="hCoverageType"]').val(); 
							if(hospital == "Hospitalisation"){
							/*	var strTypeofCov 		= $(this).find("input").val();
								var strbooster  		= $("#plancoveragetbl tbody tr").find("td").find('input[name="mulHSP"]').val();
								var strmaaAmtAsd  		= $("#plancoveragetbl tbody tr").find("td").find('input[name="maaHSP"]').val();
								var strmaaExpPerd 		= $("#plancoveragetbl tbody tr").find("td").find('input[name="expHSP"]').val();
								var strnoOfYrs 			= "";
								var strDsbltyBenf 		= $("#plancoveragetbl tbody tr").find("td").find('input[name="typHSBenefit"]').val();
								var stryrsOfDsbltyPay	= "";
								var strmaxPayoutAge		= $("#plancoveragetbl tbody tr").find("td").find('input[name="yrPayHSP"]').val();
								var strtypesOfHS  		= "";
								var strdeductible 		= $("#plancoveragetbl tbody tr").find("td").find('input[name="dedHSBenefit"]').val();
								var strcoInsurance 		= $("#plancoveragetbl tbody tr").find("td").find('input[name="insHSBenefit"]').val();
								var strdescription		= $("#plancoveragetbl tbody tr").find("td").find('input[name="desHSBenefit"]').val();
								...
								var strmaxPayoutAge		= "";
								var strtypesOfHS  		= "";
								var strdeductible 		= $("#dedHSBenefit").val();
								var strcoInsurance 		= $("#insHSBenefit").val();
								var strdescription		= $("#desHSBenefit").val();
								var strremarks 			= $("#remarks7").val();
								var strCovPKID 			= $("#hhospbenfPK").val();
								var strlongTermdisADL	= "";
								if(strTypeofCov != undefined && strTypeofCov != "undefined" && 
										strTypeofCov != "Retirement_Planning" && strTypeofCov  != "Education_Planning"){ 
							...
							if($("#hRetirement").val()== "Retirement_Planning" ){
								
								setMainEditRPPlanTbltoLifeRP();
								addCashValueToRet($("#retMultionret").val(),$("#retCashvalonret").val(),$("#lipRetRefId").val(),$("#covPayMtd").val(),$("#covPlanName").text(),$("#cashvalRetAge").val(),$("#retPrcnttoused").val(),$("#covRef").val(),indexTD,popupsflag);
								
							}
							if($("hEducation").val()== "Education_Planning" ){
								getPayoutsRows(null,null); 
							//alert("fetchEdcPlgPlanDetails");
...
//	Death_Benefit,Disability,TPD,CI-Advanced,CI-Early,PA,Hospitalisation,Retirement_Planning,Education_Planning
	...
	if(coverages.indexOf("Death_Benefit") != -1 && $("#divmulBootDeathDiv").is(':visible')){
	...
	if(coverages.indexOf("Disability") != -1 && $("#divdisBenefitDiv").is(':visible')){
	...
	if(coverages.indexOf("LongTermDisability") != -1 && $("#divLongTermdisBenefitDiv").is(':visible')){
		
		if(!(validateFocusFlds('plancoveragetbl','longTermdisBenefit', COV_DISBENF+" to Long Term Disability"))) return false;
		
		if(!(validateFocusFlds('plancoveragetbl','longTermdisPayout', COV_DISPAYOUT+" to Long Term Disability"))) return false;
		
		if(!(validateFocusFlds('plancoveragetbl','longTermmaxPayout', COV_MAXPAYOUT+" to Long Term Disability"))) return false;
		
		if(!(validateFocusFlds('plancoveragetbl','longTermdisADL', COV_MIN_ADL+" to Long Term Disability"))) return false;
		
	}
	
	if(coverages.indexOf("TPD") != -1 && $("#divmulTPDDiv").is(':visible')){
	...
	if(coverages.indexOf("CI-Advanced") != -1 && $("#divmulCriIllAdvDiv").is(':visible')){
	...
	if(coverages.indexOf("CI-Early") != -1 && $("#divmulCriIllErlyDiv").is(':visible')){
	...
	if(coverages.indexOf("PA") != -1 && $("#divmulPADiv").is(':visible')){
...
//		if(!(validateFocusFlds('plancoveragetbl','yrPayPA', COV_NOOFYR))) return;
	...
	if(coverages.indexOf("Hospitalisation") != -1 && $("#divmulHSPDiv").is(':visible')){
		//command vignesh on 24 06 2021
		...
		$("#planCoveragestbl tbody tr").each(function(){ 
			...
			if(strRefId == planPKId){
				...
				var covID = $("#planCoveragestbl tbody tr:eq("+trind+")").find("td:eq(0)").find("input:eq(1)").val();
					...
					if(reCov=="LongTermDisability"){
						$("#hlongTermdisablebenfPK").val(covID);
						$('#longTermdisBenefit').val($("#planCoveragestbl tbody tr:eq("+trind+")").find("td:eq(10)").find("input").val());
						$('#longTermdisPayout').val($("#planCoveragestbl tbody tr:eq("+trind+")").find("td:eq(11)").find("input").val());
						$('#longTermmaxPayout').val($("#planCoveragestbl tbody tr:eq("+trind+")").find("td:eq(12)").find("input").val()); 
						$('#remarks13').val($("#planCoveragestbl tbody tr:eq("+trind+")").find("td:eq(17)").find("input").val());
						$('#longTermdisADL').val($("#planCoveragestbl tbody tr:eq("+trind+")").find("td:eq(18)").find("input").val());
					}
					if(reCov=="TPD"){
					...
					if(reCov=="PA"){
						$("#hpabenfPK").val(covID);
						$('#maaPA').val($("#planCoveragestbl tbody tr:eq("+trind+")").find("td:eq(10)").find("input").val());
...
//	$("#expPA").val($("#retSelfCoordinateage").val()).prop("readonly",true);
...
function multibenefits(elm){
    ...
    $('#divmulTPD,#divmaaTPD,#divexpTPD,#divdisTPD,#divdisPayoutTPD,#divmaxPayoutTPD,#divmulTPDDiv').hide();
	$('#divmulBootDeath,#divmaaDeath,#divexpDeath,#divyrPayDeath,#divmulBootDeathDiv').hide();
	$('#divdisBenefit,#divdisPayout,#divmaxPayout,#divdisBenefitDiv,#divLongTermdisBenefit,#divmaxPayout,#divlongdisPayout,#divdisminnoofADL,#divLongTermdisBenefitDiv').hide();
	$('#divmulCriIllErly,#divmaaCriIllErly,#divexpCriIllErly,#divyrPayCriIllErly,#divmulCriIllErlyDiv').hide();
	$('#divmulCriIllAdv,#divmaaCriIllAdv,#divexpCriIllAdv,#divdisCriIllAdv,#divdisPayoutCriIllAdv,#divmaxPayoutCriIllAdv,#divmulCriIllAdvDiv').hide();
	$('#divdesHSBenefit,#divinsHSBenefit,#divdedHSBenefit,#divtypHSBenefit,#divyrPayHSP,#divexpHSP,#divmaaHSP,#divmulHSP,#divmulHSPDiv').hide();
	$('#divmulPA,#divmaaPA,#divexpPA,#divyrPayPA,#divmulPADiv').hide();
	$('#divremarks1,#divremarks2,#divremarks3,#divremarks4,#divremarks5,#divremarks6,#divremarks7,#divremarks13').hide();
	$('#btnForRetirementClone,#btnForRetirementCloneDiv').hide();
	$('#btnForEducationClone,#btnForEducationCloneDiv').hide();
	...
	$(elm).find('option').each(function() {
		...
		if(txt != undefined && txt != "undefined" &&  txt != "Retirement" && txt  != "Education"){
			...
			case "HP":casetxt = "Hospitalisation";break;
			case "PA":casetxt = "PA";break;
			case "LTDIS":casetxt = "LongTermDisability";break;
...
// 	    loadAgeStartEnd($("#txtFldDlgRetPlgCommOfAge"),$("#retAgeBasedon"));//off this validation
    ...
    if($.inArray("EP",SelCovValOfIns) != -1){ 
    	$(elm).multiselect('deselect','EP');      
      	$(elm).multiselect('select','EP'); 
 ...
 function MultiselectCoverage(elmId,hid,options,arrayList){
			...
			if(options=="ALL"){ 
				hid.val("RP,EP,TPD,DB,DIS,LTDIS,CIE,CIA,HP,PA"); 
...
//	 	$('#'+elmtId).multiselect('refresh');
 ...
 function getliBasRidDetRows(dataset,tab){
	if(dataset != null){
		...
		if(dataset["strFPMSBasicOrRider"]) {
			prodTypeFipa = dataset["strFPMSBasicOrRider"]
		}  
		
		if(dataset["basorrid"]) {
			prodTypeFipa = dataset["basorrid"]
		}
	...
	var dateTopay ="",stillNeedTopay=false;
	 ...
	 if(!isEmpty(paymentmtd) && stillNeedTopay ){ //&& paymode != "SINGLE"
		...
		if( isCPFRelated( paymentmtd )){
			//alert("Cpf Payment method----"+paymentmtd);
...
//			$tblCpfRow.find("td:eq(0)").find('input:eq(2)').addClass("rowrefid").val($rowref); 
			$tblCpfRow.find("td:eq(0)").find('input[name="txtFldCADRefId"]').addClass("rowrefid").val($rowref);   
...
//			if(pkid.indexOf("AS_") == 0){
				...
				if(isEmpty($tblCpfRow.find("td:eq(0)").find('input[name="cdPkid"]').val())){ 
					//$tblCpfRow.find("td:eq(0)").find('input:eq(1)').val(makeid(17)); 
					$tblCpfRow.find("td:eq(0)").find('input[name="cdPkid"]').val(makeid(17)); 
				}
...
//			}
			 ...
			 /*selectNullvalChk($tblCpfRow.find("td:eq(3)"),ownership);   */
		  	 selectNullvalChkBySelectName($tblCpfRow.find("td:eq(2)").find('select[name="selCdApplicantType"]'),ownership);
			...
			/* if(paymode == "ANNUALLY"){frequency="1";}
			 ...
			 selectNullvalChkBySelectName($tblCpfRow.find("td:eq(3)").find('select[name="selCADType"]'),"Life Insurance");
			...
			/* selectNullvalChk($tblCpfRow.find("td:eq(5)"),"Deduction"); */
	 		 selectNullvalChkBySelectName($tblCpfRow.find("td:eq(4)").find('select[name="selCADTypesOfTrans"]'),"Deduction");
	
			 $tblCpfRow.find("td:eq(4)").find('input[name="txtCADCpfAcctype"]').val(ccedid);  
			/* selectNullvalChk($tblCpfRow.find("td:eq(6)"),ccedid); */
			 selectNullvalChkBySelectName($tblCpfRow.find("td:eq(4)").find('select[name="selCADCpfAcctype"]'),ccedid);
			...
			$tblCpfRow.find("td:eq(4)").find('span').html( addboldtagselCADCpfAcctype +'<br>' + "Deduction" ); //Proposed
			 ...
			 /*$tblCpfRow.find("td:eq(5)").find('input[name="txtFldCADPerFrom"]').val(periodFrm);
			 $tblCpfRow.find("td:eq(5)").find('input[name="txtFldCADPerTo"]').val(selfAge);*/
			 $tblCpfRow.find("td:eq(5)").find('input[name="txtFldCADPerFrom"]').val(periodFrm);
			 $tblCpfRow.find("td:eq(5)").find('input[name="txtFldCADPerTo"]').val(dateTopay);
			 var toage = paymode == 'SINGLE' ? 0 : getYears(periodFrm, dateTopay);
			/* $tblCpfRow.find("td:eq(5)").find('input[name="txtFldCADAgeFrom"]').val(dateTopay);
			 $tblCpfRow.find("td:eq(5)").find('input[name="txtFldCADAgeTo"]').val( Number(selfAge) + Number(toage) );*/
			 $tblCpfRow.find("td:eq(5)").find('input[name="txtFldCADAgeFrom"]').val(selfAge); 
			 $tblCpfRow.find("td:eq(5)").find('input[name="txtFldCADAgeTo"]').val(Number(selfAge) + Number(toage));
			 var selfToAge=Number(selfAge) + Number(toage);
			
			
			 $tblCpfRow.find("td:eq(5)").find('span').html("Start @ "+selfAge  +'<br>' +"End @ "+  selfToAge ); //Proposed
			
			
			
//			 frequency ="1";//Always CPF Set to Annual because the amount is converted to Annual.
			 
			 $tblCpfRow.find("td:eq(6)").find('input[name="txtFldCADAmt"]').val(roundNumber(amount*Number(frequency))); 
			/* selectNullvalChk($tblCpfRow.find("td:eq(10)"),paymode == 'SINGLE'? "-1"  : "1"); ////Always CPF Set to Annual because the amount is converted to Annual.*/
			  selectNullvalChkBySelectName($tblCpfRow.find("td:eq(6)").find('select[name="selCADPayTerm"]'),paymode == 'SINGLE'? "-1"  : "1");
	 		 var addboldtagselCADAmt = '<span style="font-weight: bold;">'+"$ " + roundNumber(amount*Number(frequency)) +' '+ paymode+ '</span>';
			 var incRate=0; 
...
//			}
			...
			$tblSRSRow.find("td:eq(3)").find('select[name="selSrsClassify"]').val("Annual Withdrawals"); //Classification 
	    	$tblSRSRow.find("td:eq(3)").find('select[name="selSrsTypeTrans"]').val("Withdrawals");  	 
		
			//$tblSRSRow.find("td:eq(3)").find('input:eq(0)').val("Life Insurance/Premium"); 
			 $tblSRSRow.find("td:eq(2)").find('input[name="txtFldSrsDesc"]').val("Life Insurance/Premium"); //Description of Investment
 			 var addboldtagtxtFldCADDescription = '<span style="font-weight: bold;">' + "Life Insurance/Premium"+ '</span>';	    
			...
			//$tblSRSRow.find("td:eq(4)").find('select:eq(0)').val(paymode != "SINGLE" ? "ANNUAL" : "SINGLE"); 
			 $tblSRSRow.find("td:eq(5)").find('select[name="selSrsFreq"]').val(paymode != "SINGLE" ? "ANNUALLY" : "SINGLE");
	         
			//$tblSRSRow.find("td:eq(5)").find('input:eq(0)').val(roundNumber( Number(amount) * Number(frequency) )); 
		     $tblSRSRow.find("td:eq(5)").find('input[name="txtFldSrsAmount"]').val(roundNumber( Number(amount) * Number(frequency) ));//Amount 	 
 			
		
		     var addboldtagtxtFldAmt = '<span style="font-weight: bold;">'+"$"+ roundNumber( Number(amount) * Number(frequency) ) +" "+(paymode != ("SINGLE") ? "ANNUAL" : "SINGLE")+ '</span>';	
			...
			//$tblSRSRow.find("td:eq(6)").find('input:eq(0)').val(periodFrm);//Period From
		     //var fromyear=getdobtoage(periodFrm, true);
             var fromyear= getdobLife(periodFrm, true);
 			//$tblSRSRow.find("td:eq(4)").find('input[name="txtFldSrsPerdFrom"]').val(fromyear);//Period From	

			//$tblSRSRow.find("td:eq(7)").find('input:eq(0)').val(dateTopay);
		 	//var toyear=getdobtoage(dateTopay, true);
			var toyear=getdobLife(dateTopay, true);
			//$tblSRSRow.find("td:eq(4)").find('input[name="txtFldSrsPerdTo"]').val(toyear);//Period To	
		   
			$tblSRSRow.find("td:eq(4)").find('input[name="txtFldSrsPerdFrom"]').val(periodFrm);//Period From	date 
			$tblSRSRow.find("td:eq(4)").find('input[name="txtFldSrsPerdFromDate"]').val(fromyear);//Period From	 
		
			$tblSRSRow.find("td:eq(4)").find('input[name="txtFldSrsPerdTo"]').val(dateTopay);//Period To date
			$tblSRSRow.find("td:eq(4)").find('input[name="txtFldSrsPerdToDate"]').val(toyear);//Period To	 
		
			
 			var addboldtagtxtFldSRSAgeFrom = '<span style="font-weight: bold;">'+"Start @ "+ fromyear +'</span>';
		    var addboldtagtxtFldSRSAgeTo = '<span style="font-weight: bold;">'+"End @ "+ toyear +'</span>';
            $tblSRSRow.find("td:eq(4)").find('span').html(addboldtagtxtFldSRSAgeFrom+'<br>' +addboldtagtxtFldSRSAgeTo); //Proposed


		
			/*$tblSRSRow.find("td:eq(5)").find('input[name="txtFldSrsAgestart"]').val(fromyear);//Age Start	
			$tblSRSRow.find("td:eq(5)").find('input[name="txtFldSrsPayoutPerd"]').val(toyear);//Pay Period To	*/
	 ...
	 var paymode = $("#cvplnleftpaneltbl tbody tr:eq(8)").find("td:eq("+tdpos+")").find("select:eq(0)").val(); 
		...
		var selfage = $("#dfSelfAge").val();
			...
			$('#liRetirementPlgtbl tbody tr').each(function(){
				...
				//var $mode = $tblRetplgRow.find("td:eq(0)").find("input:eq(0)").val(); //LifeMVRet Ref Id
				var $mode = $tblRetplgRow.find("td:eq(0)").find('input[name=txtFldretplgMode]').val(); 
				
			//	var $rowref = $tblRetplgRow.find("td:eq(0)").find("input:eq(1)").val(); //LifeMVRet Ref Id
				var $rowref = $tblRetplgRow.find("td:eq(0)").find('input[name=txtFldLifeMvRetId]').val(); 
				
				//var $planrefid = $tblRetplgRow.find("td:eq(0)").find("input:eq(2)").val();
				var $planrefid = $tblRetplgRow.find("td:eq(0)").find('input[name=txtFldMvRetRefId]').val();
...
//								 deleteRetPlanMultiByRefId($rowref);
								...
								//var rowexist = chkRPIncAssetRowExist(afterret);
								var $tblRETRow = null; 
								if(rowexist == null){
								...
								$tblRETRow.find("td:eq(0)").find('input[name=txtFldIncAssRefId]').addClass("rowrefid").val($rowref);
								
								//var pkid = $tblRetplgRow.find("td:eq(0)").find('input:eq(1)').val();
								var pkid = $tblRetplgRow.find("td:eq(0)").find('input[name=txtFldLifeMvRetId]').val();
							//	if(!isEmpty(pkid)){ 
								//	if(isEmpty($tblRETRow.find("td:eq(0)").find('input:eq(1)').val())){
										if(isEmpty($tblRETRow.find("td:eq(0)").find('input[name=txtFldIncAssId]').val())){
										//$tblRETRow.find("td:eq(0)").find('input:eq(1)').val(makeid(17));
										$tblRETRow.find("td:eq(0)").find('input[name=txtFldIncAssId]').val(makeid(17));
									}
							//	}
...
//										 prevamount = amtOfInc;
								...
								$tblRETRow.find("td:eq(3)").find('select[name=txtFldIncAssClsfy]').val(clasfic);
								console.log("descpt----------->"+descpt);
								$tblRETRow.find("td:eq(2)").find('input[name=txtFldIncAssDesc]').val(descpt);
								 
								selectNullvalChkBySelectName($tblRETRow.find("td:eq(4)").find('select[name=selIncAssFreq]'),"SINGLE"); 
								
								$tblRETRow.find("td:eq(4)").find('input[name=txtFldIncAssAmtofInc]').val(retamount); 	
								
								$tblRETRow.find("td:eq(4)").find('input[name=txtFldIncAssEslrate]').val(escRate); 
									
								$tblRETRow.find("td:eq(4)").find('input[name=txtFldIncAssRoi]').val(Number("0"));   
							  
								selectNullvalChkBySelectName($tblRETRow.find("td:eq(5)").find('select[name=selIncAssAgeBsOn]'),basedon);//Need to check
							 
								$tblRETRow.find("td:eq(5)").find('input[name=txtFldIncAssAgePaySts]').val(intretslfage);
							 
								$tblRETRow.find("td:eq(5)").find('input[name=txtFldIncAssAgePayends]').val(intretslfage).prop("disabled",true);
								
								$tblRETRow.find("td:eq(4)").find('span').html("$" + retamount + " " + "SINGLE" + '<br>' + escRate + "% Escalation" +'<br>'+ Number("0") +"% ROI" ); //Proposed
								
								$tblRETRow.find("td:eq(5)").find('span').html("Start @ " + intretslfage + '<br>'); //Proposed
								...
								if(ageIncEnd > intretslfage){
									 ...
									 if(escRate == 0 ){
											...
											if(!isEmpty(pkid)){
										 	...
										 	$tblAfterRETRow.find("td:eq(3)").find('select[name=txtFldIncAssClsfy]').val(clasfic);
											console.log("descpt----------->"+descpt);
											$tblAfterRETRow.find("td:eq(2)").find('input[name=txtFldIncAssDesc]').val(descpt);
											 
											selectNullvalChkBySelectName($tblAfterRETRow.find("td:eq(4)").find('select[name=selIncAssFreq]'),"REGULAR"); 
											
											$tblAfterRETRow.find("td:eq(4)").find('input[name=txtFldIncAssAmtofInc]').val(amtOfInc); 	
											
											$tblAfterRETRow.find("td:eq(4)").find('input[name=txtFldIncAssEslrate]').val(escRate); 
												
											$tblAfterRETRow.find("td:eq(4)").find('input[name=txtFldIncAssRoi]').val(Number("0"));   
										  
											selectNullvalChkBySelectName($tblAfterRETRow.find("td:eq(5)").find('select[name=selIncAssAgeBsOn]'),basedon);//Need to check
										 
											$tblAfterRETRow.find("td:eq(5)").find('input[name=txtFldIncAssAgePaySts]').val(Number(intretslfage)+1);
										 
											$tblAfterRETRow.find("td:eq(5)").find('input[name=txtFldIncAssAgePayends]').val(ageIncEnd);
										 
									    	$tblAfterRETRow.find("td:eq(4)").find('span').html("$" + amtOfInc + " " + "REGULAR" + '<br>' + escRate + "% Escalation" +'<br>'+ Number("0") +"% ROI" ); //Proposed
						
											$tblAfterRETRow.find("td:eq(5)").find('span').html("Start @ " + Number(intretslfage)+1 + '<br>' + "End @ " + ageIncEnd); //Proposed
									 }else{
...
//										 for(var dt = intretslfage+1;dt<=ageIncEnd ;dt++){
										...
										//alert("afterret---------------"+afterret);
											 var rowexistA65 = chkRPIncAssetRowExist(afterret);
												...
												//$tblAfterRETRow.find("td:eq(0)").find('input:eq(2)').addClass("rowrefid").val(afterret);
												$tblAfterRETRow.find("td:eq(0)").find('input[name=txtFldIncAssRefId]').addClass("rowrefid").val(afterret);
												//var pkid = $tblRetplgRow.find("td:eq(0)").find('input:eq(1)').val();
												var pkid = $tblRetplgRow.find("td:eq(0)").find('input[name=txtFldLifeMvRetId]').val();
//												if(!isEmpty(pkid)){
													if(isEmpty($tblAfterRETRow.find("td:eq(0)").find('input[name=txtFldIncAssId]').val())){
														$tblAfterRETRow.find("td:eq(0)").find('input[name=txtFldIncAssId]').val(makeid(17));
													}
//												}
												...
												selectNullvalChkBySelectName($tblAfterRETRow.find("td:eq(4)").find('select[name=selIncAssFreq]'),"SINGLE"); 
												
												$tblAfterRETRow.find("td:eq(4)").find('input[name=txtFldIncAssAmtofInc]').val(Math.round(z));
												...
												$tblAfterRETRow.find("td:eq(5)").find('input[name=txtFldIncAssAgePaySts]').val(dt);
											 
												$tblAfterRETRow.find("td:eq(5)").find('input[name=txtFldIncAssAgePayends]').val(ageIncEnd).prop('disabled',true);
												
												$tblAfterRETRow.find("td:eq(4)").find('span').html("$" + Math.round(z) + " " + "SINGLE" + '<br>' + escRate + "% Escalation" +'<br>'+ Number("0") +"% ROI" ); //Proposed
										 
												$tblAfterRETRow.find("td:eq(5)").find('span').html("Start @ " + dt + '<br>'); //Proposed
//										 }
...
//									var pkid = $tblRetplgRow.find("td:eq(0)").find('input:eq(1)').val();
								...
								//	}
									...
									/* selectNullvalChk($tblCpfRow.find("td:eq(3)"),owner); */
									  selectNullvalChkBySelectName($tblCpfRow.find("td:eq(2)").find('select[name="selCdApplicantType"]'),owner);
									 
								     $tblCpfRow.find("td:eq(2)").find('input[name="txtFldCADDescription"]').val(descpt);
							
								 var addboldtagtxtFldCADDescription = '<span style="font-weight: bold;">' + descpt + '</span>';
		     					 var applicant= $tblCpfRow.find("td:eq(2)").find('select[name="txtFldCADApplicant"] option:selected').text();
		    					 $tblCpfRow.find("td:eq(2)").find('span').html(addboldtagtxtFldCADDescription  +'<br>' + applicant + '<br>'  +  owner); //Proposed
									...
									selectNullvalChkBySelectName($tblCpfRow.find("td:eq(4)").find('select[name="selCADTypesOfTrans"]'),"Addition");
								    ...
								    $tblCpfRow.find("td:eq(4)").find('input[name="txtCADCpfAcctype"]').val(ccedid);  
			 						...
			 						selectNullvalChkBySelectName($tblCpfRow.find("td:eq(4)").find('select[name="selCADCpfAcctype"]'),ccedid);
										   ...
										   var acctype= $tblCpfRow.find("td:eq(4)").find('select[name="selCADCpfAcctype"] option:selected').text();
		   								   var addboldtagselCADCpfAcctype = '<span style="font-weight: bold;">' + acctype + '</span>';
										  $tblCpfRow.find("td:eq(4)").find('span').html( addboldtagselCADCpfAcctype +'<br>' + "Addition" ); //Proposed
...
//										 prevamount = amtOfInc;
								 ...
								 //alert("perdfrom---------"+perdfrom);
								 // alert("intretslfage---------"+intretslfage);
							
								 $tblCpfRow.find("td:eq(5)").find('input[name="txtFldCADPerFrom"]').val(perdfrom);//date
								 $tblCpfRow.find("td:eq(5)").find('input[name="txtFldCADPerTo"]').val(perdfrom);//date
								 
//								 var toage = paymode == 'SINGLE' ? 0 : getYears(perdfrom, dateTopay);
								 $tblCpfRow.find("td:eq(5)").find('input[name="txtFldCADAgeFrom"]').val(intretslfage);//age 
								 $tblCpfRow.find("td:eq(5)").find('input[name="txtFldCADAgeTo"]').val( intretslfage ).prop("disabled",true);//age
								
								
								
			 					$tblCpfRow.find("td:eq(5)").find('span').html("Start @ "+intretslfage  +'<br>' +"End @ "+  intretslfage ); //Proposed
		
		
		
								 frequency ="1";//Always CPF Set to Annual because the amount is converted to Annual.
								 
								 $tblCpfRow.find("td:eq(6)").find('input[name="txtFldCADAmt"]').val(roundNumber(retamount)); 
								/* selectNullvalChk($tblCpfRow.find("td:eq(10)"),"-1"); */
								 selectNullvalChkBySelectName($tblCpfRow.find("td:eq(6)").find('select[name="selCADPayTerm"]'),"-1");
								
								 var paymode= $tblCpfRow.find("td:eq(6)").find('select[name="selCADPayTerm"] option:selected').text();
								 var addboldtagselCADAmt = '<span style="font-weight: bold;">'+"$ " + roundNumber(retamount) +' '+ paymode+ '</span>';
			 					 var incRate=0; 
			 					 $tblCpfRow.find("td:eq(6)").find('span').html(addboldtagselCADAmt  +'<br>' + incRate+"% increment"); //Proposed
		
								
								if(ageIncEnd > intretslfage){
									//alert("==========ageIncEnd > intretslfage======");
									 if(escRate == 0 ){
										// alert("==========escRate == 0======");
...
//											}
												...
												//	$tblAfterRETRow.find("td:eq(2)").find('option:eq(1)').prop("selected","selected");
													$tblAfterRETRow.find("td:eq(2)").find('select[name="txtFldCADApplicant"]').find('option:eq(1)').prop("selected","selected");
												 }else  if(owner == "Spouse"){
													// $tblAfterRETRow.find("td:eq(2)").find('option:eq(2)').prop("selected","selected");
													 $tblAfterRETRow.find("td:eq(2)").find('select[name="txtFldCADApplicant"]').find('option:eq(2)').prop("selected","selected");
												 }else{
													// $tblAfterRETRow.find("td:eq(2)").find('option:eq(0)').prop("selected","selected");
													$tblAfterRETRow.find("td:eq(2)").find('select[name="txtFldCADApplicant"]').find('option:eq(0)').prop("selected","selected");
												 } 
												 
												 /*selectNullvalChk($tblAfterRETRow.find("td:eq(3)"),owner); */
										 
									  			selectNullvalChkBySelectName($tblAfterRETRow.find("td:eq(2)").find('select[name="selCdApplicantType"]'),owner);
									 
								     			$tblAfterRETRow.find("td:eq(2)").find('input[name="txtFldCADDescription"]').val(descpt);
			
											 	var addboldtagtxtFldCADDescription = '<span style="font-weight: bold;">' + descpt + '</span>';
		     								 	var applicant= $tblAfterRETRow.find("td:eq(2)").find('select[name="txtFldCADApplicant"] option:selected').text();
		    									 $tblAfterRETRow.find("td:eq(2)").find('span').html(addboldtagtxtFldCADDescription  +'<br>' + applicant + '<br>'  +  owner); //Proposed
												...
												$tblAfterRETRow.find("td:eq(4)").find('input:eq(0)').val(planname);  
												 ...
												 selectNullvalChkBySelectName($tblAfterRETRow.find("td:eq(3)").find('select[name="selCADType"]'),"Life Insurance");
												 $tblAfterRETRow.find("td:eq(3)").find('input[name="txtLifeCpfPlanName"]').val(planname);  
											 	 $tblAfterRETRow.find("td:eq(3)").find('input[name="txtLifeCpfPolNo"]').val(policyNo); 
												 $tblAfterRETRow.find("td:eq(3)").find('div[id="spanplanname"]').text(isEmpty(planname) ? "": "Plan Name : "+planname);  
												 $tblAfterRETRow.find("td:eq(3)").find('div[id="spanpolno"]').text(isEmpty(policyNo) ? "": "Policy No : "+policyNo);
											...
											selectNullvalChkBySelectName($tblAfterRETRow.find("td:eq(4)").find('select[name="selCADTypesOfTrans"]'),"Addition");
										    $tblAfterRETRow.find("td:eq(4)").find('input[name="txtCADCpfAcctype"]').val(ccedid);  
					 						selectNullvalChkBySelectName($tblAfterRETRow.find("td:eq(4)").find('select[name="selCADCpfAcctype"]'),ccedid);
							
							
										   var acctype= $tblAfterRETRow.find("td:eq(4)").find('select[name="selCADCpfAcctype"] option:selected').text();
		   								   var addboldtagselCADCpfAcctype = '<span style="font-weight: bold;">' + acctype + '</span>';
										  $tblAfterRETRow.find("td:eq(4)").find('span').html( addboldtagselCADCpfAcctype +'<br>' + "Addition" ); //Proposed
...
//											 var perdfrom =  addyearsToDate(ResultNewDate, Number(intretslfage)+1);
								 ...
								 var retagediff = (Number(intretslfage)+1) - Number(selfage);
								 var perdfrom =  addyearsToDate(ResultNewDate, retagediff);
								//  alert("retagediff---------"+retagediff);
								//  alert("perdfrom---------"+perdfrom);
								 $tblAfterRETRow.find("td:eq(5)").find('input[name="txtFldCADPerFrom"]').val(perdfrom);//date
								...
								//alert("perdTo---------"+perdTo);
								//alert("diffto---------"+diffto);
								$tblAfterRETRow.find("td:eq(5)").find('input[name="txtFldCADPerTo"]').val(perdTo);//date  
								 $tblAfterRETRow.find("td:eq(5)").find('input[name="txtFldCADAgeFrom"]').val(Number(intretslfage)+1);//age
								 $tblAfterRETRow.find("td:eq(5)").find('input[name="txtFldCADAgeTo"]').val( ageIncEnd );//age
								
								
			 					$tblAfterRETRow.find("td:eq(5)").find('span').html("Start @ "+ Number(intretslfage)+1  +'<br>' +"End @ "+ ageIncEnd  ); //Proposed
		
		
								 $tblAfterRETRow.find("td:eq(6)").find('input[name="txtFldCADAmt"]').val(roundNumber(amtOfInc)); 
								
								 selectNullvalChkBySelectName($tblAfterRETRow.find("td:eq(6)").find('select[name="selCADPayTerm"]'),"1");
								
								 var paymode= $tblAfterRETRow.find("td:eq(6)").find('select[name="selCADPayTerm"] option:selected').text();
								 var addboldtagselCADAmt = '<span style="font-weight: bold;">'+"$ " + roundNumber(amtOfInc) +' '+ paymode+ '</span>';
			 					 var incRate=0; 
			 					 $tblAfterRETRow.find("td:eq(6)").find('span').html(addboldtagselCADAmt  +'<br>' + incRate+"% increment"); //Proposed
...
//												var pkid = $tblRetplgRow.find("td:eq(0)").find('input:eq(1)').val();
												...
												selectNullvalChkBySelectName($tblAfterRETRow.find("td:eq(2)").find('select[name="selCdApplicantType"]'),owner);	
		    									 ...
		    									 $tblAfterRETRow.find("td:eq(2)").find('span').html(addboldtagtxtFldCADDescription  +'<br>' + applicant + '<br>'  +  owner); //Proposed
													...
													var cprarr = getCPFAccDets(paymtd);// paymentmtd 09/12/2022
													var cpfedacc=cprarr[0],ccedid=cprarr[1];
...
//													 var perdTo =  addyearsToDate(ResultNewDate, diffto);
								...
								// alert("perdfrom---------"+perdfrom);
								// alert("Number(dt)---------"+Number(dt));
								var pt = Number(ageIncEnd) - (Number(intretslfage)+1);
							    var perdto =  addyearsToDate(perdfrom, pt);
								//alert("perdto--------"+perdto);
								//alert("ageIncEnd--------"+ageIncEnd);
								$tblAfterRETRow.find("td:eq(5)").find('input[name="txtFldCADPerTo"]').val(perdto);//date 
								 $tblAfterRETRow.find("td:eq(5)").find('input[name="txtFldCADAgeFrom"]').val(Number(dt));
								 $tblAfterRETRow.find("td:eq(5)").find('input[name="txtFldCADAgeTo"]').val( ageIncEnd ).prop('disabled',true);
								
								
			 					$tblAfterRETRow.find("td:eq(5)").find('span').html("Start @ "+Number(dt)  +'<br>' +"End @ "+  ageIncEnd ); //Proposed
								...
								$tblAfterRETRow.find("td:eq(6)").find('input[name="txtFldCADAmt"]').val(roundNumber(z)); 
								
								 selectNullvalChkBySelectName($tblAfterRETRow.find("td:eq(6)").find('select[name="selCADPayTerm"]'),"-1");
								
								 var paymode= $tblAfterRETRow.find("td:eq(6)").find('select[name="selCADPayTerm"] option:selected').text();
								 var addboldtagselCADAmt = '<span style="font-weight: bold;">'+"$ " + roundNumber(z) +' '+ paymode+ '</span>';
			 					 var incRate=0; 
			 					 $tblAfterRETRow.find("td:eq(6)").find('span').html(addboldtagselCADAmt  +'<br>' + incRate+"% increment"); //Proposed
...
//										 prevamount = amtOfInc;
								 ...
								 // $tblSRSRowR.find("td:eq(2)").find('select:eq(0)').val("Annual Addition"); //Classification  
								  $tblSRSRowR.find("td:eq(3)").find('select[name="selSrsClassify"]').val("Annual Addition"); //Classification 
	    						  $tblSRSRowR.find("td:eq(3)").find('select[name="selSrsTypeTrans"]').val("Addition");  	 
								
								
									//$tblSRSRowR.find("td:eq(3)").find('input:eq(0)').val("Life Insurance/Retirement"); 
								    $tblSRSRowR.find("td:eq(2)").find('input[name="txtFldSrsDesc"]').val("Life Insurance/Retirement"); //Description of Investment
 			                        var addboldtagtxtFldCADDescription = '<span style="font-weight: bold;">' + "Life Insurance/Retirement"+ '</span>';	    
 			                        $tblSRSRowR.find("td:eq(2)").find('span').html(addboldtagtxtFldCADDescription); //Proposed
									
									//$tblSRSRowR.find("td:eq(4)").find('select:eq(0)').val("SINGLE"); 
									$tblSRSRowR.find("td:eq(5)").find('select[name="selSrsFreq"]').val("SINGLE");
									
									
									/*$tblSRSRowR.find("td:eq(5)").find('input[name="txtFldSrsAgestart"]').val(intretslfage);//Age Start	
									$tblSRSRowR.find("td:eq(5)").find('input[name="txtFldSrsPayoutPerd"]').val(intretslfage);//Pay Period To*/	
			
									//$tblSRSRowR.find("td:eq(5)").find('input:eq(0)').val(roundNumber( retamount )); 
							    	$tblSRSRowR.find("td:eq(5)").find('input[name="txtFldSrsAmount"]').val(roundNumber( retamount ));//Amount 	 
									var addboldtagtxtFldAmt = '<span style="font-weight: bold;">'+"$"+ roundNumber( retamount ) +" "+"SINGLE"+ '</span>';	
		     					    $tblSRSRowR.find("td:eq(5)").find('span').html(addboldtagtxtFldAmt); //Proposed
 									
								    //alert("perdfrom-------"+perdfrom);
								    // var fromyear=getdobtoage(perdfrom, true);
									//$tblSRSRowR.find("td:eq(6)").find('input:eq(0)').val(perdfrom);//Period From
							    	$tblSRSRowR.find("td:eq(4)").find('input[name="txtFldSrsPerdFrom"]').val(perdfrom);//Period From date
									$tblSRSRowR.find("td:eq(4)").find('input[name="txtFldSrsPerdFromDate"]').val(intretslfage);
									
									
									
									//$tblSRSRowR.find("td:eq(7)").find('input:eq(0)').val(perdfrom);
								 	$tblSRSRowR.find("td:eq(4)").find('input[name="txtFldSrsPerdTo"]').val(perdfrom);//Period To date
								    $tblSRSRowR.find("td:eq(4)").find('input[name="txtFldSrsPerdToDate"]').val(intretslfage); //age
								   
									
 									var addboldtagtxtFldSRSAgeFrom = '<span style="font-weight: bold;">'+"Start @ "+ intretslfage +'</span>';
		   							var addboldtagtxtFldSRSAgeTo = '<span style="font-weight: bold;">'+"End @ "+ intretslfage +'</span>';
           							$tblSRSRowR.find("td:eq(4)").find('span').html(addboldtagtxtFldSRSAgeFrom+'<br>' +addboldtagtxtFldSRSAgeTo); //Proposed
...
//											}
												...
												$tblAfterRETRow.find("td:eq(3)").find('select[name="selSrsClassify"]').val("Annual Addition"); //Classification 
	    										$tblAfterRETRow.find("td:eq(3)").find('select[name="selSrsTypeTrans"]').val("Addition");  	

												//$tblAfterRETRow.find("td:eq(3)").find('input:eq(0)').val("Life Insurance/Retirement"); 
											    $tblAfterRETRow.find("td:eq(2)").find('input[name="txtFldSrsDesc"]').val("Life Insurance/Retirement"); //Description of Investment
 												 var addboldtagtxtFldCADDescription = '<span style="font-weight: bold;">' + "Life Insurance/Retirement"+ '</span>';	    
 			 									$tblAfterRETRow.find("td:eq(2)").find('span').html(addboldtagtxtFldCADDescription); //Proposed
												
												//$tblAfterRETRow.find("td:eq(4)").find('select:eq(0)').val("SINGLE"); 
												$tblAfterRETRow.find("td:eq(5)").find('select[name="selSrsFreq"]').val("SINGLE");
												
												//$tblAfterRETRow.find("td:eq(5)").find('input:eq(0)').val(roundNumber( amtOfInc )); 
												$tblAfterRETRow.find("td:eq(5)").find('input[name="txtFldSrsAmount"]').val(roundNumber( amtOfInc ));//Amount 	 
												
												 var addboldtagtxtFldAmt = '<span style="font-weight: bold;">'+"$"+ roundNumber( amtOfInc ) +" "+"SINGLE"+ '</span>';	
		    									 $tblAfterRETRow.find("td:eq(5)").find('span').html(addboldtagtxtFldAmt); //Proposed
										  ...
										  // alert("perdTo------"+perdTo);
												...
												// $tblAfterRETRow.find("td:eq(6)").find('input:eq(0)').val(perdfrom);//Period From
												 // var fromyear=getdobtoage(perdfrom, true);
										   		 $tblAfterRETRow.find("td:eq(4)").find('input[name="txtFldSrsPerdFrom"]').val(perdfrom);//Period From date	
												$tblAfterRETRow.find("td:eq(4)").find('input[name="txtFldSrsPerdFromDate"]').val(intretslfage);
								
												// $tblAfterRETRow.find("td:eq(7)").find('input:eq(0)').val(perdTo);
												// var toyear=getdobtoage(perdTo, true);
												 $tblAfterRETRow.find("td:eq(4)").find('input[name="txtFldSrsPerdTo"]').val(perdTo);//Period To	date
										 		 $tblAfterRETRow.find("td:eq(4)").find('input[name="txtFldSrsPerdToDate"]').val(ageIncEnd);
											
												var addboldtagtxtFldSRSAgeFrom = '<span style="font-weight: bold;">'+"Start @ "+ intretslfage +'</span>';
		   									    var addboldtagtxtFldSRSAgeTo = '<span style="font-weight: bold;">'+"End @ "+ ageIncEnd +'</span>';
                                                $tblAfterRETRow.find("td:eq(4)").find('span').html(addboldtagtxtFldSRSAgeFrom+'<br>' +addboldtagtxtFldSRSAgeTo); //Proposed

												/*$tblAfterRETRow.find("td:eq(5)").find('input[name="txtFldSrsAgestart"]').val(intretslfage);//Age Start	
									            $tblAfterRETRow.find("td:eq(5)").find('input[name="txtFldSrsPayoutPerd"]').val(ageIncEnd);//Pay Period To*/	
									 }else{
...
//												}
													...
													$tblAfterRETRow.find("td:eq(5)").find('select[name="selSrsFreq"]').val("ANNUALLY");
												
													//$tblAfterRETRow.find("td:eq(5)").find('input:eq(0)').val(roundNumber( roundNumber(z) )); 
													$tblAfterRETRow.find("td:eq(5)").find('input[name="txtFldSrsAmount"]').val(roundNumber( roundNumber(z) ));//Amount 	 
													
													/*$tblAfterRETRow.find("td:eq(5)").find('input[name="txtFldSrsAgestart"]').val(Number(ageIncEnd) + Number(selfage));//Age Start	
									            	$tblAfterRETRow.find("td:eq(5)").find('input[name="txtFldSrsPayoutPerd"]').val(ageIncEnd);//Pay Period To	*/

													var addboldtagtxtFldAmt = '<span style="font-weight: bold;">'+"$"+ roundNumber( roundNumber(z) ) +" "+"ANNUAL"+ '</span>';	
		     										$tblAfterRETRow.find("td:eq(5)").find('span').html(addboldtagtxtFldAmt); //Proposed
...
//													 var perdTo =  addyearsToDate(ResultNewDate, diffto);
													...
													// $tblAfterRETRow.find("td:eq(6)").find('input:eq(0)').val(perdfrom);//Period From
													  $tblAfterRETRow.find("td:eq(4)").find('input[name="txtFldSrsPerdFrom"]').val(perdfrom);//Period From	date
													  $tblAfterRETRow.find("td:eq(4)").find('input[name="txtFldSrsPerdFromDate"]').val(Number(ageIncEnd) + Number(selfage));//age
													...
													// $tblAfterRETRow.find("td:eq(7)").find('input:eq(0)').val(perdTo);
													 $tblAfterRETRow.find("td:eq(4)").find('input[name="txtFldSrsPerdTo"]').val(perdTo);//Period To date
												 	 $tblAfterRETRow.find("td:eq(4)").find('input[name="txtFldSrsPerdToDate"]').val(ageIncEnd);//age
											        
													var endAge=Number(ageIncEnd) + Number(selfage);
													var addboldtagtxtFldSRSAgeFrom = '<span style="font-weight: bold;">'+"Start @ "+ endAge +'</span>';
		    										var addboldtagtxtFldSRSAgeTo = '<span style="font-weight: bold;">'+"End @ "+ ageIncEnd +'</span>';
           											 $tblAfterRETRow.find("td:eq(4)").find('span').html(addboldtagtxtFldSRSAgeFrom+'<br>' +addboldtagtxtFldSRSAgeTo); //Proposed
...
//			 $("#lipRetRefId").val(rowRefID);
			 ...
			 $tblRetRow.find("td:eq(3)").find('select[name=txtFldIncAssClsfy]').val(cls);

			 $tblRetRow.find("td:eq(3)").find('input[name=txtFldIncAsstSubClsfy]').val("DISIMBURSEMENT");
console.log("descpt----------->"+descpt);
			 $tblRetRow.find("td:eq(2)").find('input[name=txtFldIncAssDesc]').val(descpt);			

			 $tblRetRow.find("td:eq(4)").find('select[name=selIncAssFreq]').val("SINGLE"); 		 	

			 $tblRetRow.find("td:eq(4)").find('input[name=txtFldIncAssAmtofInc]').val(amountofincome);

			 $tblRetRow.find("td:eq(4)").find('input[name=txtFldIncAssEslrate]').val("").attr("readonly",true);			

			 $tblRetRow.find("td:eq(4)").find('input[name=txtFldIncAssRoi]').val(Number("0"));  			

			 $tblRetRow.find("td:eq(5)").find('select[name=selIncAssAgeBsOn]').val(basedon);//Need to check  

			 $tblRetRow.find("td:eq(5)").find('input[name=txtFldIncAssAgePaySts]').val(intretslfage);

			 $tblRetRow.find("td:eq(5)").find('input[name=txtFldIncAssAgePayends]').val("").attr("readonly",true);
...
//			}
				...
				//$tblSRSRowR.find("td:eq(5)").find('input:eq(0)').val(roundNumber( amountofincome )); 
		   		$tblSRSRowR.find("td:eq(5)").find('input[name="txtFldSrsAmount"]').val(roundNumber( amountofincome ));//Amount 	 
					

				var addboldtagtxtFldAmt = '<span style="font-weight: bold;">'+"$"+ roundNumber( amountofincome ) +" "+"SINGLE"+ '</span>';	
		    	 $tblSRSRowR.find("td:eq(5)").find('span').html(addboldtagtxtFldAmt); //Proposed

				var fromyear=getdobtoage(perdfrom, true);
				//$tblSRSRowR.find("td:eq(6)").find('input:eq(0)').val(perdfrom);//Period From
		   		$tblSRSRowR.find("td:eq(4)").find('input[name="txtFldSrsPerdFrom"]').val(perdfrom);//Period From	
				
				var toyear=getdobtoage(perdfrom, true);
				//$tblSRSRowR.find("td:eq(7)").find('input:eq(0)').val(perdfrom);
				$tblSRSRowR.find("td:eq(4)").find('input[name="txtFldSrsPerdTo"]').val(perdfrom);//Period To
				
				$tblSRSRowR.find("td:eq(4)").find('input[name="txtFldSrsPerdFromDate"]').val(fromyear);
				$tblSRSRowR.find("td:eq(4)").find('input[name="txtFldSrsPerdToDate"]').val(toyear);
				
			   var addboldtagtxtFldSRSAgeFrom = '<span style="font-weight: bold;">'+"Start @ "+ fromyear +'</span>';
		       var addboldtagtxtFldSRSAgeTo = '<span style="font-weight: bold;">'+"End @ "+ toyear +'</span>';
               $tblSRSRowR.find("td:eq(4)").find('span').html(addboldtagtxtFldSRSAgeFrom+'<br>' +addboldtagtxtFldSRSAgeTo); //Proposed

				/*$tblSRSRowR.find("td:eq(5)").find('input[name="txtFldSrsAgestart"]').val(fromyear);//Age Start	
				$tblSRSRowR.find("td:eq(5)").find('input[name="txtFldSrsPayoutPerd"]').val(toyear);//Pay Period To*/
			...
			$tblCpfRowR.find("td:eq(0)").find('input:eq(2)').addClass("rowrefid").val(retrefid);
				...
				if(ownership == "Self"){
					/*$tblCpfRowR.find("td:eq(2)").find('option:eq(1)').prop("selected","selected");*/
					 $tblCpfRowR.find("td:eq(2)").find('select[name="txtFldCADApplicant"]').find('option:eq(1)').prop("selected","selected");
				 }else  if(ownership == "Spouse"){
					/* $tblCpfRowR.find("td:eq(2)").find('option:eq(2)').prop("selected","selected");*/
				 $tblCpfRowR.find("td:eq(2)").find('select[name="txtFldCADApplicant"]').find('option:eq(2)').prop("selected","selected");
				 }else{
					/* $tblCpfRowR.find("td:eq(2)").find('option:eq(0)').prop("selected","selected");*/
				 $tblCpfRowR.find("td:eq(2)").find('select[name="txtFldCADApplicant"]').find('option:eq(0)').prop("selected","selected");
				 } 
				 
				  
				 /*selectNullvalChk($tblCpfRowR.find("td:eq(3)"),ownership); */ 
				 selectNullvalChkBySelectName($tblCpfRowR.find("td:eq(2)").find('select[name="selCdApplicantType"]'),ownership); 
				/* selectNullvalChk($tblCpfRowR.find("td:eq(4)"),"Life Insurance"); */ 
				 selectNullvalChkBySelectName($tblCpfRowR.find("td:eq(3)").find('select[name="selCADType"]'),"Life Insurance");
				 $tblCpfRowR.find("td:eq(3)").find('input[name="txtLifeCpfPlanName"]').val(planname);  
				 $tblCpfRowR.find("td:eq(3)").find('input[name="txtLifeCpfPolNo"]').val(policyNo);  
				 
				 $tblCpfRowR.find("td:eq(3)").find('div[id="spanplanname"]').text(isEmpty(planname) ? "": "Plan Name : "+planname);  
				 $tblCpfRowR.find("td:eq(3)").find('div[id="spanpolno"]').text(isEmpty(policyNo) ? "": "Policy No : "+policyNo);  
				 
				 /*selectNullvalChk($tblCpfRowR.find("td:eq(5)"),"Addition"); */
			 selectNullvalChkBySelectName($tblCpfRowR.find("td:eq(4)").find('select[name="selCADTypesOfTrans"]'),"Addition");
					
				 $tblCpfRowR.find("td:eq(4)").find('input[name="txtCADCpfAcctype"]').val(ccid);  
				/* selectNullvalChk($tblCpfRowR.find("td:eq(6)"),ccid); */
				 selectNullvalChkBySelectName($tblCpfRowR.find("td:eq(4)").find('select[name="selCADCpfAcctype"]'),ccid);
			
				 $tblCpfRowR.find("td:eq(5)").find('input[name="txtFldCADPerFrom"]').val(perdfrom);
				// $tblCpfRowR.find("td:eq(5)").find('input[name="txtFldCADPerTo"]').val(cashvalretage);
				 $tblCpfRowR.find("td:eq(5)").find('input[name="txtFldCADPerTo"]').val(perdfrom);
//				 var toage = getYears(perdfrom, dateTopay);
				// $tblCpfRowR.find("td:eq(5)").find('input[name="txtFldCADAgeFrom"]').val(perdfrom);
			     $tblCpfRowR.find("td:eq(5)").find('input[name="txtFldCADAgeFrom"]').val(cashvalretage);
				 $tblCpfRowR.find("td:eq(5)").find('input[name="txtFldCADAgeTo"]').val(cashvalretage );
			 
				
				 
				 frequency ="-1";//Always CPF Set to Annual because the amount is converted to Annual.
				 
				 $tblCpfRowR.find("td:eq(6)").find('input[name="txtFldCADAmt"]').val(roundNumber( amountofincome )); 
				/* selectNullvalChk($tblCpfRowR.find("td:eq(10)"),frequency);*/ 
				 selectNullvalChkBySelectName($tblCpfRowR.find("td:eq(6)").find('select[name="selCADPayTerm"]'),frequency);
...
//	console.log("plan sync id------->"+cashrefid+",planpkid-->"+planpkid)
	...
	// add vignesh on 24 06 2021
//	$("#expPA").val(isEmpty(slfretage)?$("#retSelfCoordinateage").val():slfretage);
...
function ilifeInsPremium(opts,flg){ 
...
//				console.log("i==>"+i+"val===>"+val)
				if(i=="CLNT_LIFEPREM_AMT"){
					    var policydataFPMS = [];//aravindh
					    var policydataFIPA = [];//aravindh
					    $.each(val, function(contkey, contvalue) { 
					    	var fpmsAppName = contvalue["jsonApplnName"];
					    	var lifeRefId 	= contvalue["jsonLifeId"];
					    	if(fpmsAppName == "FPMSNL"){
						    	policydataFPMS.push(lifeRefId);
					    	}
					    });
...
//						var planstatus = contvalue["jsonLifePlanStatus"];
						var policyno = contvalue["jsonLifePlanPolNo"]; //aravindh
					...
					/*	else{//aravindh -- always add in array dont check if els part
							if(appName == "FPMSNL"){ 
								policydataFPMS.push(policyno);//aravindh ok proceed ok i wil check here APPID
						 	 }
							else{
								policydataFIPA.push(policyno);//aravindh
							}
						}*/
						
						/*var positions = [];//aravindh
						var dupliPolicy;
						  for(var i=0;i<policydataFPMS.length;i++){//aravindh
						  var result = [];
						       for(var j=0;j<policydataFIPA.length;j++){
						          if(policydataFPMS[i] == policydataFIPA[j]){ // dont check equal, u should check the value is exists or not
						        	  result.push(policydataFPMS[i]); 
						        	  dupliPolicy=policydataFPMS[i];
						        	  alert(" for dupliPolicy---"+dupliPolicy);
						          }
						       }
						...
						/* original code aravindh*/
						
								if(isCashRelated(paymtd)){
									
								
								
								 if(!isEmpty(premAmt) && isValidObject(premAmt) && !checkArryValueExist(policydataFPMS,lifeId)){ //here appid flag exit can i check in condition
...
//								 calcTotalPlandetails(opts,strPremAmtSelf,strPremAmtSpouse,strPremAmtFamily);
	...
	calcTotalPlandetails(opts,flg,strPremAmtSelf,strPremAmtSpouse,strPremAmtFamily);
...
function calcTotalPlandetails(opts,flg,sumSlf,sumSps,sumFam){ 
		 	 var sumOfSA=0; 
		     ...
		     if(opts == "FETCH"){
		            ...
		            if(flg){
					     ...
					     */
					     ...
					     $("#expdSelfInsuranceCACL").val(roundNumber(sumSlf)); 
						 $("#expdSpsInsuranceCACL").val(roundNumber(sumSps));
					     $("#expdFamilyInsuranceCACL").val(roundNumber(sumFam));
					}
...
//	 if(ind.length >0){
	 $("#cvplnleftpaneltbl tbody tr:eq(0) td").each( function( index, value ) {   
	 	...
	 	paymtd = isEmpty(paymtd) ? "CASH" : paymtd;
	 	if(paymtd == "CASH"){
	 		$("#cvplnleftpaneltbl tbody tr:eq(9)").find("td:eq("+index+")").find('select:eq(0) option[value="CASH"]').prop("selected",true);
	 	}else{
	 		$("#cvplnleftpaneltbl tbody tr:eq(9)").find("td:eq("+index+")").find('select:eq(0) option[value="'+paymtd+'"]').prop("selected",true);
//	 		$('#selPaymtdB00 option[value="'+paymtd+'"]').prop("selected",true);
	 	}
	 	//$('#selPaymtdB00 option[value="'+paymtd+'"]').prop("selected",true);
	 	 if(isCashRelated(paymtd)){
...
//	 }
 ...
 $("#lipSa").val(roundNumber(sumOfSA));  
 $("#lipTotalSa").val(Number(sumOfSA).toFixed(2)); 
 $("#lipTotalPrem").val(Number(totpremium).toFixed(2));  
