...
var cell7 ='<input type="text" name="attachremarks" class="form-control editable"  onmouseover="fipaTooltip(this);" maxlength="300"/>'
...
$("#DcAttchDRow").on("click",function(){ 
	attachmentdatatableDeleteRow('fnaAttachments',fnaAttachments,'SelfnaAttachments','','DcAttchDRow'); 
});

 
function attachmentdatatableDeleteRow(tblId,datatbl,tblSel,eIcon,dIcon){ 
		...
		$('#confirmationalertmsgdiv').find(".modal-footer").find("button:eq(0)").click(function (){   
			$('#'+tblId+' tbody').find('input[type=checkbox],input[type=radio]').each(function(){
				if($(this).is(":checked")){
					...
					reOrderAttachemntmentVisibleRows(tblId);
