...
$("a[href^='#liabParticulars']").trigger('click')
//	$($.fn.dataTable.tables(true)).DataTable().columns.adjust().draw(false);	
...
var cell2 = '<input type="text" name="txtFldPropCpfOwnDesc" maxlength="300" class="form-control editable form-tablecolor" onmouseover="fipaTooltip(this);" style="width:190px"/>';

var cell3 = '<input type="text" name="txtFldPropCpfMktVal" class="form-control editable form-tablecolor"   onmouseover="fipaTooltip(this);"  />';

var cell4 = '<select   name="txtFldPropCpfOwnership" class="form-control editable form-tablecolor"  ></select>';

var cell5 = '<input type="text" name="txtFldPropCpfApprecrate" class="form-control editable applyEvntpCent3 form-tablecolor" onmouseover="fipaTooltip(this);" onchange="callCalcPropFV($(this));syncPropTblRow($(this).closest(\'tr\'));" />'; 

var cell6 = '<input type="text" name="txtFldPropCpfMortageos" class="form-control editable form-tablecolor" onmouseover="fipaTooltip(this);"  />'; 

var cell7 = '<input type="text" name="txtFldPropCpfOsloanPerd" class="form-control editable form-tablecolor" onmouseover="fipaTooltip(this);"  />';
 
var cell8 = '<select   name="txtFldPropCpfObj" class="form-control editable form-tablecolor"  ></select>';

var cell9 = '<input type="text" name="txtFldPropCurAnlRetInc" class="form-control editable form-tablecolor"   onmouseover="fipaTooltip(this);" />'; 


var cell10 = '<input type="text" name="txtFldPropRentPerd" class="form-control editable form-tablecolor"   onmouseover="fipaTooltip(this);" />';


var cell11 = '<input type="text" name="txtFldPropCpfLoanBySlfCpf" class="form-control editable form-tablecolor"   onmouseover="fipaTooltip(this);"  />';

var cell12 = '<input type="text" name="txtFldPropCpfLoanBySlfCash" class="form-control editable form-tablecolor"   onmouseover="fipaTooltip(this);" />';

var cell13 = '<input type="text" name="txtFldPropCpfLoanBySpsCpf" class="form-control editable clsfipaSpouse form-tablecolor"   onmouseover="fipaTooltip(this);"  />';

var cell14 = '<input type="text" name="txtFldPropCpfLoanBySpsCash" class="form-control editable clsfipaSpouse form-tablecolor"   onmouseover="fipaTooltip(this);" />';

var cell15 = '<select  name="txtFldPropCpfSold" class="form-control editable form-tablecolor" ></select>'; 

var cell16 = '<select  name="txtFldPropRentOnRetire" class="form-control editable form-tablecolor"  ></select>';

var cell17 ='<input type="text" name="txtFldPropCurRetInc" class="form-control editable form-tablecolor"   onmouseover="fipaTooltip(this);"  />'+
'<input type="hidden" name="txtFldPropCpfCrtdBy"/><input type="hidden" name="txtFldPropCpfCrtdDate"/>';

var cell18 = '<input type="text" name="txtFldPropYrsToRent" class="form-control editable form-tablecolor"   onmouseover="fipaTooltip(this);"  />';

var cell19 = '<input type="text" name="txtFldPropFvOnRent" class="form-control editable form-tablecolor" readonly="true"  onmouseover="fipaTooltip(this);"  />';
...
$lastRow.find("td:eq(0)").find('input[name=txtFldPropRefId]').val($("#txtFldDlgPropRefId").val());
		...
		var infoDetsArr = new Array();
			...
			for(var data in dataset){
				...
				case "propDescription": 
					...
					propertyDescriptionarr.push(""+col+"");
					break;
...
//	  $('#propOwnCpf_Dialog #txtFldDlgPropMode').val($lastRow.find("td:eq(0)").find('input:eq(0)').val());//instant save added line
	  //$('#propOwnCpf_Dialog #txtFldDlgCpfPropOwnId').val($lastRow.find("td:eq(0)").find('input:eq(1)').val());
 	  $('#propOwnCpf_Dialog #txtFldDlgCpfPropOwnId').val($lastRow.find("td:eq(0)").find('input[name=txtFldPropCpfOwnId]').val());
	  //$('#propOwnCpf_Dialog #txtFldDlgPropRefId').val($lastRow.find("td:eq(0)").find('input:eq(2)').val());
//alert("load id from row---"+$lastRow.find("td:eq(0)").find('input[name=txtFldPropRefId]').val());
	$('#propOwnCpf_Dialog #txtFldDlgPropRefId').val($lastRow.find("td:eq(0)").find('input[name=txtFldPropRefId]').val());
...
function propfilldomval($RowId,$row){ 
	...
	$row.find("td:eq(0)").find('input[name=txtFldPropCpfOwnId]').val($('#txtFldDlgCpfPropOwnId').val());
	//$row.find("td:eq(0)").find('input:eq(2)').val($('#txtFldDlgPropRefId').val());
	$row.find("td:eq(0)").find('input[name=txtFldPropRefId]').val($('#txtFldDlgPropRefId').val())
...
//	console.log("syncPropTblRow")
	...
	var pkid = $tblPropRow.find("td:eq(0)").find('input[name=txtFldPropCpfOwnId]').val();
	//var proprefid = $tblPropRow.find("td:eq(0)").find('input:eq(2)').val();
	var proprefid = $tblPropRow.find("td:eq(0)").find('input[name=txtFldPropRefId]').val();
	//var proprefid = $tblPropRow.find("td:eq(0)").find('input[name=txtFldPropCpfOwnId]').val();
	var rowRefID = pkid ;//"PROP_"+$tblPropRow.index();
		//alert("proprefid-----"+proprefid);
	...
	if((onretirement == "N" && torent=="Y") || onretirement=="Y"){
		...
		$tblPropRow.find("td:eq(0)").find('input[name=txtFldPropRefId]').addClass("rowrefid").val(proprefid); 
		//$tblPropRow.find("td:eq(0)").find('input:eq(2)').addClass("rowrefid").val(rowRefID); 
		//$tblRetRow.find("td:eq(0)").find('input[name=txtFldIncAssRefId]').addClass("rowrefid").val(rowRefID);
		$tblRetRow.find("td:eq(0)").find('input[name=txtFldIncAssRefId]').addClass("rowrefid").val(proprefid);
//		if(isEmpty(pkid)){
...
//		}
		...
		$tblRetRow.find("td:eq(3)").find('select[name=txtFldIncAssClsfy]').val("Properties");	
		 console.log("propdesc----------->"+propdesc);
		$tblRetRow.find("td:eq(2)").find('input[name=txtFldIncAssDesc]').val(isEmpty(propdesc)?"":propdesc);	
		$tblRetRow.find("td:eq(4)").find('select[name=selIncAssFreq]').val(freq);
		 
	 	
	 	if(onretirement=="Y"){
	 		$tblRetRow.find("td:eq(4)").find('input[name=txtFldIncAssAmtofInc]').val((isEmpty(propfvonrent)? Number(0): Number(propfvonrent)));
	 	}else{
	 		$tblRetRow.find("td:eq(4)").find('input[name=txtFldIncAssAmtofInc]').val((isEmpty(estfuturerent)? Number(0): Number(estfuturerent)));
	 	}
		...
		if(freq == "SINGLE"){
			$tblRetRow.find("td:eq(5)").find('input[name=txtFldIncAssAgePayends]').prop("disabled",true);
			$tblRetRow.find("td:eq(5)").find('input[name=txtFldIncAssAgePayends]').val(""); 
			
		}else{
			$tblRetRow.find("td:eq(5)").find('input[name=txtFldIncAssAgePayends]').prop("disabled",false);
			$tblRetRow.find("td:eq(5)").find('input[name=txtFldIncAssAgePayends]').val((isEmpty(intretslfage)) ? Number("0") : Number(intretslfage)+Number(yrtorent)); 
...
//		}
		...
		$tblCpfRow.find("td:eq(2)").find('input[id="txtFldCADDescription"]').val(description);
		...
		selectNullvalChkBySelectName($tblCpfRow.find("td:eq(3)").find('select[name="selCADType"]'),"Loan Repayment");
		$tblCpfRow.find("td:eq(3)").find('input[id="txtLifeCpfPlanName"]').val(isEmpty(propdesc) ? "": propdesc);
		$tblCpfRow.find("td:eq(3)").find('div[id="spanplanname"]').text(isEmpty(propdesc) ? "": "Property : "+propdesc);
		...
		selectNullvalChkBySelectName($tblCpfRow.find("td:eq(4)").find('select[name="selCADTypesOfTrans"]'),"Deduction");
	     $tblCpfRow.find("td:eq(4)").find('input[name="txtCADCpfAcctype"]').val("Ordinary");  
		 selectNullvalChkBySelectName($tblCpfRow.find("td:eq(4)").find('select[name="selCADCpfAcctype"]'),"CPFACC000001");
			
		var addboldtagselCADCpfAcctype = '<span style="font-weight: bold;">' + "Ordinary" + '</span>';
...
//		$tblCpfRow.find("td:eq(6)").find('input:eq(0)').val("Ordinary");
		 ...
		 $tblCpfRow.find("td:eq(5)").find('span').html("Start @ "+"-"  +'<br>' +"End @ "+  "-"); //Proposed

		$tblCpfRow.find("td:eq(6)").find('input[id="txtFldCADAmt"]').val(ClntLnCpf);
		$tblCpfRow.find("td:eq(6)").find('select[name="selCADPayTerm"]').val(1);
		var freqTransaction=$tblCpfRow.find("td:eq(6)").find('select[name="selCADPayTerm"] option:selected').text(); 
		var addboldtagselCADAmt = '<span style="font-weight: bold;">'+"$ " + ClntLnCpf +' '+ freqTransaction+ '</span>';
		var incRate=0; 
...
//vignesh 02-03-2021   //common component pdf

$("#btnpropertydivPrint").click(function (){ 
	
	
	
var rl = fnaPropOwnTblByCPF.row().count();
...
//	var rl = $('#fnaInvestmentTbl tbody tr:visible').length;
	...
	var fileSave ="PROPERTY_";
	var screentittle ="Properties"; // $('#screentittle').val();  //screen title
	var fnaPropOwnTblByCPFtblId ="fnaPropOwnTblByCPF";  //table id
	var pdfTitle =""; // table title name
	//var colheaderarray =["#","Description","Market Value ($)","Ownership","Property Appreciation Rate (%)","Mortgage Outstanding ($)","Outstanding Loan Period (yrs)","Objective","Cur.Annl.Rental Income ($)","Rental Period (yrs)","Client's Annual Loan Repayment (By CPF ($))","Client's Annual Loan Repayment (By Cash ($))","Spouse's Annual Loan Repayment (By CPF ($))","Spouse's Annual Loan Repayment (By Cash ($))","On Retirement (Sold)","On Retirement (To Rent)","On Retirement (Est. Future Annl. Rental Income ($))","On Retirement (Years to Rent (yrs))","On Retirement (FV of Property on retirement ($))"];          
	//var colheaderarray =["#","Description","Market Value ($)","Ownership","Property Appreciation Rate (%)","Mortgage Outstanding ($)","Outstanding Loan Period (yrs)","Objective","Cur.Annl.Rental Income ($)","Rental Period (yrs)","By CPF ($)","By Cash ($)","By CPF ($)","By Cash ($)","Sold","To Rent","Est. Future Annl. Rental Income ($)","Years to Rent (yrs)","FV of Property on retirement ($)"];
	var screentittlefootername = "Properties";
	...
	var colheaderarray  = [
	    [
	        {content: '', colSpan: 10, styles: {halign: 'center'}}, 
	    ...
	    ["#","Description","Market Value($)","Ownership","Property Appreciation Rate(%)","Mortgage Outstanding($)","Outstanding Loan Period(yrs)","Objective","Cur.Annl.Rental Income($)","Rental Period(yrs)","By CPF($)","By Cash($)","By CPF($)","By Cash($)","Sold","To Rent","Est. Future Annl. Rental Income($)","Years to Rent(yrs)","FV of Property on retirement($)"],
	];
	
	var jsonObject = 
	    [{"key":"Table","value":fnaPropOwnTblByCPFtblId,"Title":pdfTitle,"TitleFooter":screentittlefootername,"header":colheaderarray,"pdfaddnewPage":""} 
         ];
