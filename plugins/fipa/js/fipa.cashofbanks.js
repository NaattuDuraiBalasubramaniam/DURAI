...
//092021 - move these to navigation.js 
...
$("#CobARow").on("click",function(){
	...
	//if(!valCobTbl())return; 
			...
			$("#txtFldDlgDescription").val("Cash At Bank");
...
'<select name="txtFldMainAccHolderName" class="form-control editable form-tablecolor"  onmouseover="fipaTooltip(this);"  style="display:none"></select>'+
...
var cell3 = '<select  name="selCOBAccountType" class="form-control editable form-tablecolor fontbold"   onmouseover="fipaTooltip(this);" readonly="readonly" disabled="disabled"/></select>'+
...
var cell4 = '<input type="hidden" name="txtFldCOBCurBalance" class="form-control editable form-tablecolor fontbold"   onmouseover="fipaTooltip(this);" readonly="readonly"/>'+
'<span  onmouseover="fipaTooltip(this);"  style="width: 130px; word-wrap: break-word;display: inline-block; overflow: hidden;"/></span>';//Proposed
...
'<span   onmouseover="fipaTooltip(this);"  style="width: 91px; word-wrap: break-word;display: inline-block;"/></span>';//Proposed
...
'<span   onmouseover="fipaTooltip(this);"  style="width: 135px; word-wrap: break-word;display: inline-block;"/></span>';//Proposed
/*
...
//add vignesh on 24 06 2021
//var addboldtagjqvalCAB = '<span style="font-weight: bold;">'+$("#txtFldDlgDescription").val() + '</span>'; 
 
//$lastRow.find("td:eq(2)").find('span').html(addboldtagjqvalCAB   +'<br>' +  $("#txtFldDlgMainAccHolderName").val() + ", " + '<br>'  +  $("#selDlgCOBOwnershipType").val() +" Ownership"); //Proposed


$lastRow.find("td:eq(2)").find('span').html($("#txtFldDlgDescription").val()  +'<br>' +  $("#txtFldDlgMainAccHolderName").val() + ", " + '<br>'  +  $("#selDlgCOBOwnershipType").val() +" Ownership"); //Proposed
...
//$lastRow.find("td:eq(4)").find('span').text("$" + $("#txtFldDlgCOBCurBalance").val()); //Proposed
var curBal=(isEmpty($("#txtFldDlgCOBCurBalance").val())) ? Number("0") : Number($("#txtFldDlgCOBCurBalance").val());
$lastRow.find("td:eq(4)").find('span').text("$" + curBal); //Proposed
...
//$lastRow.find("td:eq(5)").find('span').html($("#selDlgCOBObjective").val() + '<br>' +" @ " +  $("#txtFldDlgCOBRetrmntPrcnt").val() + "%"); //Proposed 
var retPer=(isEmpty($("#txtFldDlgCOBRetrmntPrcnt").val())) ? Number("100") : Number($("#txtFldDlgCOBRetrmntPrcnt").val());

var addboldtagjqval = '<span style="font-weight: bold;">'+ $("#selDlgCOBObjective").val() + '</span>'; 

if($("#selDlgCOBObjective").val() =="Child Education"){
	$lastRow.find("td:eq(5)").find('span').html(addboldtagjqval); //Proposed
}else{
	$lastRow.find("td:eq(5)").find('span').html(addboldtagjqval + '<br>' +" @ " +  retPer + "%"); //Proposed
}


//$lastRow.find("td:eq(5)").find('span').html($("#selDlgCOBObjective").val() + '<br>' +" @ " +  retPer + "%"); //Proposed 
...
//$lastRow.find("td:eq(6)").find('span').html("$" + $("#txtFldDlgCOBRegDeposit").val() + " " + $("#selDlgCOBDepositFreq").val()  +", "+ '<br>' + "For " + $("#txtFldDlgCOBPeriodRegDeposit").val() + " years"); //Proposed
var regdep=(isEmpty($("#txtFldDlgCOBRegDeposit").val())) ? Number("0") : Number($("#txtFldDlgCOBRegDeposit").val());
var regPerdep=(isEmpty($("#txtFldDlgCOBPeriodRegDeposit").val())) ? Number("0") : Number($("#txtFldDlgCOBPeriodRegDeposit").val());

//add vignesh on 24 06 2021
var addboldtagregdep = '<span style="font-weight: bold;">'+"$" + regdep + " " + $("#selDlgCOBDepositFreq").val() + '</span>'; 
 
$lastRow.find("td:eq(6)").find('span').html(addboldtagregdep   +", "+ '<br>' + "For " + regPerdep + " years"); //Proposed

//$lastRow.find("td:eq(6)").find('span').html("$" + regdep + " " + $("#selDlgCOBDepositFreq").val()  +", "+ '<br>' + "For " + regPerdep + " years"); //Proposed
applyEventHandlers();
	...
	if(dataset != null){
		...
		$lastRow.find("td:first").find('span').text(rowCount); 
			...
			for(var data in dataset){
				...
				var txtFldCOBcurrentBalance;//Proposed
				switch(data){
				...
				/*$lastRow.find("td:eq(5)").find('span').html(txtfldCOBObjetve + '<br>' + " @ " +  txtfldCOBRetrmntPrcnt + "%"); //Proposed 
				$lastRow.find("td:eq(6)").find('span').html("$" + txtFldCOBRegDeposit + " " + txtFldCOBDepositFreq  + ", " + '<br>' + "For " + txtFldCOBPeriodRegDeposit + " years"); //Proposed
				$lastRow.find("td:eq(2)").find('span').html(txtFldCOBDescription + '<br>' +  txtFldCOBmainAccountHolder + ", " + '<br>' +  txtFldCOBownershipType +" Ownership"); //Proposed
				$lastRow.find("td:eq(4)").find('span').text("$" + txtFldCOBcurrentBalance); //Proposed*/
				var retPer=(isEmpty(txtfldCOBRetrmntPrcnt)) ? Number("100") : Number(txtfldCOBRetrmntPrcnt);
				var regDep=(isEmpty(txtFldCOBRegDeposit)) ? Number("0") : Number(txtFldCOBRegDeposit);
				var perRegDep=(isEmpty(txtFldCOBPeriodRegDeposit)) ? Number("0") : Number(txtFldCOBPeriodRegDeposit);
				var curBal=(isEmpty(txtFldCOBcurrentBalance)) ? Number("0") : Number(txtFldCOBcurrentBalance);
				
				//add vignesh on 24 06 2021
				var addboldtagtxtfldCOBObjetve = '<span style="font-weight: bold;">'+ txtfldCOBObjetve+'</span>';
				if($("#selDlgCOBObjective").val() =="Child Education"){
					$lastRow.find("td:eq(5)").find('span').html(addboldtagtxtfldCOBObjetve); //Proposed 
				}else{
					$lastRow.find("td:eq(5)").find('span').html(addboldtagtxtfldCOBObjetve + '<br>' + " @ " +  retPer + "%"); //Proposed 
				}
				//$lastRow.find("td:eq(5)").find('span').html(addboldtagtxtfldCOBObjetve + '<br>' + " @ " +  retPer + "%"); //Proposed 
				
				//add vignesh on 24 06 2021
				var addboldtagtxtFldCOBDepositFreq = '<span style="font-weight: bold;">'+"$" + regDep + " " + txtFldCOBDepositFreq+'</span>';//"$" + regDep + " " + $("#txtFldCOBDepositFreq").val()
				 
				$lastRow.find("td:eq(6)").find('span').html(addboldtagtxtFldCOBDepositFreq + ", " + '<br>' + "For " + perRegDep + " years"); //Proposed
				
				//$lastRow.find("td:eq(6)").find('span').html("$" + regDep + " " + txtFldCOBDepositFreq  + ", " + '<br>' + "For " + perRegDep + " years"); //Proposed
				
				//add vignesh on 24 06 2021
				var addboldtagtxtFldCOBDescription = '<span style="font-weight: bold;">'+txtFldCOBDescription+'</span>';//"$" + regDep + " " + $("#txtFldCOBDepositFreq").val()
			 
				$lastRow.find("td:eq(2)").find('span').html(addboldtagtxtFldCOBDescription + '<br>' +  txtFldCOBmainAccountHolder + ", " + '<br>' +  txtFldCOBownershipType +" Ownership"); //Proposed
...
//						instantCobSave(null,DEL_MODE); //instant save added line	
				...
				reOrderCashAtbankVisibleRows("cashOfBanksTable");
...
//	  $('#Cob_Dialog #txtFldDlgCashBankMode').val($lastRow.find("td:eq(0)").find('input:eq(0)').val());//instant save added line
	  ...
	  var curBal=(isEmpty($lastRow.find("td:eq(4)").find('input:eq(0)').val())) ? Number("0") : Number($lastRow.find("td:eq(4)").find('input:eq(0)').val());
	  $('#Cob_Dialog #txtFldDlgCOBCurBalance').val(curBal);
	  
	  var retPer=(isEmpty($lastRow.find("td:eq(5)").find('input:eq(0)').val())) ? Number("0") : Number($lastRow.find("td:eq(5)").find('input:eq(0)').val());
	  $('#Cob_Dialog #txtFldDlgCOBRetrmntPrcnt').val(retPer);
	  
	  var regdep=(isEmpty($lastRow.find("td:eq(6)").find('input:eq(0)').val())) ? Number("0") : Number($lastRow.find("td:eq(6)").find('input:eq(0)').val());
	  $('#Cob_Dialog #txtFldDlgCOBRegDeposit').val(regdep);
	  
	  var perregdep=(isEmpty($lastRow.find("td:eq(6)").find('input:eq(7)').val())) ? Number("0") : Number($lastRow.find("td:eq(6)").find('input:eq(7)').val());
	  $('#Cob_Dialog #txtFldDlgCOBPeriodRegDeposit').val(perregdep);
...
function Cobfilldomval($RowId,$row){
	...
	/*$row.find("td:eq(5)").find('span').html($("#selDlgCOBObjective").val() + '<br>' +" @ " +  $("#txtFldDlgCOBRetrmntPrcnt").val() + "%"); //Proposed 
	$row.find("td:eq(6)").find('span').html("$" + $("#txtFldDlgCOBRegDeposit").val() + " " + $("#selDlgCOBDepositFreq").val()  +", "+ '<br>' + "For " + $("#txtFldDlgCOBPeriodRegDeposit").val() + " years"); //Proposed
	$row.find("td:eq(2)").find('span').html($("#txtFldDlgDescription").val() + '<br>' +  $("#txtFldDlgMainAccHolderName").val() + ", " +'<br>' +  $("#selDlgCOBOwnershipType").val() +" Ownership"); //Proposed
	$row.find("td:eq(4)").find('span').text("$" + $("#txtFldDlgCOBCurBalance").val()); //Proposed*/	
	$row.find("td:eq(2)").find('span').html($("#txtFldDlgDescription").val() + '<br>' +  $("#txtFldDlgMainAccHolderName").val() + ", " +'<br>' +  $("#selDlgCOBOwnershipType").val() +" Ownership"); //Proposed
	
	var retPer=(isEmpty($("#txtFldDlgCOBRetrmntPrcnt").val())) ? Number("100") : Number($("#txtFldDlgCOBRetrmntPrcnt").val());
	var regDep=(isEmpty($("#txtFldDlgCOBRegDeposit").val())) ? Number("0") : Number($("#txtFldDlgCOBRegDeposit").val());
	var perRegDep=(isEmpty($("#txtFldDlgCOBPeriodRegDeposit").val())) ? Number("0") : Number($("#txtFldDlgCOBPeriodRegDeposit").val());
	var curBal=(isEmpty($("#txtFldDlgCOBCurBalance").val())) ? Number("0") : Number($("#txtFldDlgCOBCurBalance").val());
	...
	if($("#selDlgCOBObjective").val() =="Child Education"){
		$row.find("td:eq(5)").find('span').html(addboldtagjqval); //Proposed
	}else{
		$row.find("td:eq(5)").find('span').html(addboldtagjqval + '<br>' +" @ " +  retPer + "%"); //Proposed
	}
	/*$row.find("td:eq(5)").find('span').html(addboldtagjqval + '<br>' +" @ " +  retPer + "%"); //Proposed
*/	
	//$row.find("td:eq(5)").find('span').html($("#selDlgCOBObjective").val() + '<br>' +" @ " +  retPer + "%"); //Proposed 
	
	//add vignesh on 24 06 2021
	var addboldtagregDep = '<span style="font-weight: bold;">'+"$" + regDep + " " + $("#selDlgCOBDepositFreq").val()  + '</span>'; 
	 
	$row.find("td:eq(6)").find('span').html(addboldtagregDep +", "+ '<br>' + "For " + perRegDep + " years"); //Proposed
	
	//$row.find("td:eq(6)").find('span').html("$" + regDep + " " + $("#selDlgCOBDepositFreq").val()  +", "+ '<br>' + "For " + perRegDep + " years"); //Proposed
	
	//add vignesh on 24 06 2021
	//var addboldtagvaljq = '<span style="font-weight: bold;">'+$("#txtFldDlgDescription").val()  + '</span>'; 
	 
	//$row.find("td:eq(6)").find('span').html(addboldtagvaljq + '<br>' +  $("#txtFldDlgMainAccHolderName").val() + ", " +'<br>' +  $("#selDlgCOBOwnershipType").val() +" Ownership"); //Proposed
	
	//$row.find("td:eq(2)").find('span').html($("#txtFldDlgDescription").val() + '<br>' +  $("#txtFldDlgMainAccHolderName").val() + ", " +'<br>' +  $("#selDlgCOBOwnershipType").val() +" Ownership"); //Proposed
	$row.find("td:eq(4)").find('span').text("$" + curBal); //Proposed
	...
	if(objective == "Retirement"){
		...
		if(!(owner == "Spouse" )){
			...
			//}
				...
				var description = $tblCabRow.find("td:eq(2)").find("input[name=txtFldCOBDescription]").val();	
						 
				 var bankname = $tblCabRow.find("td:eq(3)").find("input[name=txtFldCOBBankName]").val();
				 var acctno= $tblCabRow.find("td:eq(3)").find("input[name=txtFldCOBBankAccNo]").val();
				 var currentbalance = $tblCabRow.find("td:eq(4)").find("input[name=txtFldCOBCurBalance]").val();
				 
				 var rspamt = $tblCabRow.find("td:eq(6)").find("input[name=txtFldCOBRegDeposit]").val();
				 var rspfreq = $tblCabRow.find("td:eq(6)").find("select[name=selCOBDepositFreq]").val();
				 var rspfromdate = $tblCabRow.find("td:eq(6)").find("input[name=txtFldCOBPerFrom]").val();
				 var rsptodate = $tblCabRow.find("td:eq(6)").find("input[name=txtFldCOBPerTo]").val();
				 
				 rspamt = isEmpty(rspamt)?0:Number(rspamt);
				 
				 var invobj = $tblCabRow.find("td:eq(5)").find("select[name=selCOBObjective]").val(); 
				 var retirementprcnt = $tblCabRow.find("td:eq(5)").find("input[name=txtFldCOBRetrmntPrcnt]").val();
				var yrstoret = $tblCabRow.find("td:eq(6)").find("input[name=txtFldCOBPeriodRegDep]").val();
...
//					 console.log("amountofincome--->"+amountofincome);
					...
					$tblRetRow.find("td:eq(2)").find('input[name=txtFldIncAssDesc]').val(desc); 
 					...
 					$tblRetRow.find("td:eq(3)").find('select[name=txtFldIncAssClsfy]').val("Others");
					
					$tblRetRow.find("td:eq(4)").find('select[name=selIncAssFreq]').val("SINGLE"); 	
					
					$tblRetRow.find("td:eq(4)").find('input[name=txtFldIncAssAmtofInc]').val(Math.round(amountofincome)); 
					
					$tblRetRow.find("td:eq(4)").find('input[name=txtFldIncAssEslrate]').val("");  
					
					$tblRetRow.find("td:eq(4)").find('input[name=txtFldIncAssRoi]').val("");  
					
					var amtOfInc=Math.round(amountofincome);
					var roi=Number("0");
					var eslrate=Number("0");
					$tblRetRow.find("td:eq(4)").find('span').html("$" + amtOfInc + " " + "SINGLE" + '<br>' + eslrate + "% Escalation" + '<br>'+ roi +"% ROI" ); //Proposed

					/*var owner=$("#selDlgCOBOwnershipType").val().toUpperCase(); */
					
					var owner="SELF"; 
					
					$tblRetRow.find("td:eq(5)").find('select[name=selIncAssAgeBsOn]').val(isEmpty(owner)? basedon :owner);	 
					
					$tblRetRow.find("td:eq(5)").find('input[name=txtFldIncAssAgePaySts]').val((isEmpty(intretslfage)) ? Number("0") : Number(intretslfage)); 
				 
//					$tblRetRow.find("td:eq(10)").find('input:eq(0)').val(rspfreq == "REGULAR" ? ((isEmpty(totAge)) ? Number("0") : Number(totAge)) : "");
					$tblRetRow.find("td:eq(5)").find('input[name=txtFldIncAssAgePayends]').val("").prop("readonly",true); 
					var agestarts=(isEmpty(intretslfage)) ? Number("0") : Number(intretslfage);
...
var accr = document.querySelectorAll(".panel-headingasset");
var cont = document.querySelectorAll(".panel-collapseasset");

accr.forEach(x => {

x.addEventListener("click",function(){
var li = document.getElementById("assetliabs");
setTimeout(function(){
...
cont.forEach(x => {
if(x.classList.contains("in")){
	li.innerHTML = x.previousElementSibling.querySelector("a").innerText;
