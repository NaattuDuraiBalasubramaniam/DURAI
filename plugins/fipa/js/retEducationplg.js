...
function teritaryAgeCal(chldname,terAge,infRate,childAge){
	...
	if(!isEmpty(chldname.val())){ 
		$("#childParticularsTableNew").find("[name=txtFldChldName]").each(function(){
			...
			if(tblChldname == chldname.val()){ 
				...
				var age=Number($(this).closest("tr").find("input[name=txtFldChldAge]").val());
				//var yrstoter=Number($(this).closest("tr").find("input[name=txtFldChldYrsofEdu]").val());
				var yrstoter=$(this).closest("tr").find("input[name=txtFldChldYrs2ter]").val()
...
function getPayoutsRows(dataset,indx){
	...
	var cell0 = '<span></span>'+
				'<input type="hidden" name="txtFldEduPlgMode" readonly="true" value="'+INS_MODE+'" class="fipaModeDummy"/><input name="txtFldEduPlgId" type="hidden"/><input name="txtFldEduPlgPlanName" type="hidden"/><input name="txtFldEduPlgRefId" type="hidden"/>';
		
	var cell1 ='<div class="checkbox checkbox-primary text-center"><input type="checkbox" name="radPaytSelect"/><label>&nbsp;</label></div>'; 
	 
	var cell2 ='<input type="text" name="txtFldEduPlgChlBegAge" class="form-control editable numbers applyEvntYrs" onmouseover="fipaTooltip(this);" maxlength="20"/>';

	var cell3 ='<input type="text" name="txtFldEduPlgTotProPaid" class="form-control editable numbers applyEvntUsd26" onmouseover="fipaTooltip(this);" maxlength="126"/>';

	var cell4 ='<input type="text" name="txtFldEduPlgChldTerAge" class="form-control editable numbers applyEvntUsd26" onmouseover="fipaTooltip(this);" readonly=true maxlength="126"/>';
	...
	var $lastRow =$("#liEduPayoutstbl tbody").find("tr").last();
	$lastRow.find("td:first").find('span').text(rowCount); 
	...
	if(dataset != null){
		...
		for(var data in dataset){
			...
			switch(data){
				...
				case "chliId": 
					$lastRow.find("td:eq(0)").find('input[name=txtFldEduPlgId]:last').val(col);  //alert("chliId----"+col);
					break; 

				case "chliRefId":
					$lastRow.find("td:eq(0)").find('input[name=txtFldEduPlgRefId]:last').val(col);  //alert("chliRefId----"+col);
					break; 
					
				case "childName":  
					//$lastRow.find("td:eq(2)").find('select[name=selEdPlgChldName]:last').val(col);
					$("#selDlgEdPlgChldName").val(col);
					break; 
					
				case "chliPlanName": 
					$lastRow.find("td:eq(0)").find('input[name=txtFldEduPlgPlanName]:last').val(col);  //alert("chliPlanName----"+col);
					break;
				...
				case "childAge": 
					//$lastRow.find("td:eq(6)").find('input[name=txtFldEdPlgChldAge]:last').val(col); 
					$("#txtFldDlgEdPlgChldAge").val(col);
					break; 
					
				case "terEdcAge":
					//$lastRow.find("td:eq(7)").find('input[name=txtFldEdPlgTerEdcAge]:last').val(col);  
					 $("#txtFldDlgEdPlgTerEdcAge").val(col);
					break;
					 
				case "bankIntRate": 
				//	$lastRow.find("td:eq(8)").find('input[name=txtFldEdPlgBankIntRate]:last').val(col);  
					 $("#txtFldDlgEdPlgBankIntRate").val(col); 
					break;
					
				case "loanIntRate": 
					//$lastRow.find("td:eq(9)").find('input[name=txtFldEdPlgLoanIntRate]:last').val(col);  
					 $("#txtFldDlgEdPlgLoanIntRate").val(col); 
					break;
				 
				case "inflationRate": 
					//$lastRow.find("td:eq(10)").find('input[name=txtFldEdPlgInfRate]:last').val(col);  
					 $("#txtFldDlgEdPlgInfRate").val(col); 
					break;
				 
				case "startAge": 
				//	var startAge= col;
				//	$lastRow.find("td:eq(2)").find('input[name=txtFldEduPlgChlBegAge]:last').val(startAge).attr("value",startAge);  
					$lastRow.find("td:eq(2)").find('input[name=txtFldEduPlgChlBegAge]:last').val(col);  
					break;
					
				case "paidByPolicy":
					var paidByPolicy=col;
					$lastRow.find("td:eq(3)").find('input[name=txtFldEduPlgTotProPaid]:last').val(paidByPolicy).attr("value",paidByPolicy);  
//					$lastRow.find("td:eq(2)").find('input[name=txtFldEduPlgTotProPaid]:last').val(col);  
					break;  
					
				case "convertVal": 
					var convertVal=col;
					$lastRow.find("td:eq(4)").find('input[name=txtFldEduPlgChldTerAge]:last').val(convertVal).attr("value",convertVal);  
//					$lastRow.find("td:eq(2)").find('input[name=txtFldEduPlgChldTerAge]:last').val(col);  
					break; 
		...
		$lastRow.find("td:first").find("input:eq(2)").val(strplanName);
		$lastRow.find("td:first").find("input:eq(3)").val(planRefId);
	}
	if(dataset != null){}
...
function totalPvTerEdAge(){ 
	...
	var sum=0;
	$("[name=txtFldEduPlgChldTerAge]").each(function(){
...
$("#edPaytDRow").on("click",function(){ 
	...
	$('#liEduPayoutstbl tbody').find('input[type=checkbox]').each(function(){
		...
		//	var row = $(this).parents("tr");   
			var row = $(this).closest("tr");  
			 ...
			 var refId=$(this).parents("tr").find("td:first").find("input:eq(2)").val(); 
			
			 var  childEduMode  = row.find("td:eq(0)").find("input[name=txtFldEduPlgMode]").val();
			 
			 
			 if(childEduMode == UPD_MODE){
				 row.find("td:eq(0)").find("input[name=txtFldEduPlgMode]").val(DEL_MODE);
				}else{
					row.find("td:eq(0)").find("input[name=txtFldEduPlgMode]").val("");
				 }
	...
	datatableDeleteRow('liEduPayoutstbl',liEduPayoutstbl,'','','edPaytDRow');  
	 totalPvTerEdAge();
}); 
