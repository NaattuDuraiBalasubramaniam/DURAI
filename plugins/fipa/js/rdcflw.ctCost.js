...
$(".updateAllAnalysis").on("change",function(){
 ...
 $("button#btnRPCFAnalysis").on("click",function(){
	 ...
	 if(!(retAgeBasedonFlag === "--SELECT--")){//aravindh
 ...
 $("#btnRPCFAnalysisRep").on("click",function(){
	 ...
	 setTimeout(function(){
		 showLoader();
	   //  $("#btnRetirementPlanningScreenPrint").trigger("click");
...
//		 $('#rpCashFlwAnlys_Dialog').modal('hide'); 
 ...
 function callRPCFAnalysis(){
	...
	// syncToRpcf4();// this is the repeat process of syncToRpcf3(); - it looks repeat so it is commentted - 092021
...
//button[id=btnRetOk],-> where is thi button id???092021
...
//id=btnRetReset -> this is button not used now 092021
...
//	}	 
 ...
 //move this method to retirement related js--092021
...
//remomove this method never user anywhere--092021
...
$("#selDlgprojORAgeBsOn").on("change",function(){//aravindh
	if(!rdStartAgeValidate($('#txtFldDlgprojORAgePaySts'),$('#selDlgprojORAgeBsOn')))return;//aravindh
});
$("#txtFldDlgprojORAgePaySts").on("change",function(){//aravindh
	if(!rcpfStartAgeValidate($('#txtFldRDSlfIntAge'),$("#txtFldDlgprojORAgePaySts")))return;//aravindh
	if(!rdStartAgeValidate($('#txtFldDlgprojORAgePaySts'),$('#selDlgprojORAgeBsOn')))return;//aravindh
});	
...
//aravindh
function rcpfStartAgeValidate(startAgeid,elmId){
...
//			var dupChk=[];
			...
			if($rowCount > 0 ){
				...
				$("#RDExptbl tbody tr").each(function(i,row){
					 ...
					 if (crnthd.indexOf(/\./g) == -1) {
						 crnthd=crnthd.replace(/\./g,'_') ; 
						
						}
					
//					 crnthd.replace . with _ and try
					 if(mode_r != "D"){

						 var expenditure_data = {};
						 
						
						 expenditure_data.data=crnthd+"-"+i; 
						 expenditure_data.title=crnthd;//+"-"+i;
						 expenditure_data.width="15px";
						 expenditure_data.className="dt-right";
						 expenditure_data.render= $.fn.dataTable.render.number(',', '.', 0, '$');
						  headlist.push(crnthd+"-"+i);
						 dataHeader.push(expenditure_data); 
						...
						if(headlist.length > 0){
							...
							$.each(headlist,function(j,hdr){
								...
								$("#RDExptbl tbody tr").each(function(count,value){
									...
									if ($desclt.indexOf(/\./g) == -1) {
										$desclt=$desclt.replace(/\./g,'_') ; 
...
//										arrHdr=(test)?(hdrTest+"-"+count):hdr;
										...
										if(($desclt+"-"+count) == arrHdr){
											if($agebasedlt == "SELF"){ 
												if($freqlt == "REGULAR"){
													console.log("$startslfagelt----------------->"+$startslfagelt+",$endagelt-->"+$endagelt);
													console.log("i----------------->"+i);
													if(i >= $startslfagelt &&  (i<=$endagelt)){ 
														if(i == $startslfagelt){ 
															console.log("$anlexplt----------------->"+$anlexplt);
															arrlist[arrHdr]=(isNaN($anlexplt)) ? 0 : roundNumber(Number($anlexplt));  
															console.log("arrlist[arrHdr]----------------->"+arrlist[arrHdr]);
                    ...
                    tooltips: {
                        ...
                        callbacks: {
                            ...
                            label: (tooltipItem, data) => {
                                return data.datasets[tooltipItem.datasetIndex].label + ": $ " + tooltipItem.yLabel;//tooltipItem.yLabel --092021 number format
                            }
                            ...
                            ticks: {
                                beginAtZero: true,
                                 callback: function(value, index, values) {
                                    return '$ ' + value; //092021 - numner format
                                }
                            }
                            // stacked:true//Stacked bar chart
                        } 
                ...
                options: {
                    ...
                    tooltips: {
                        ...
                        callbacks: {
                            ...
                            label: (tooltipItem, data) => {
                                return data.datasets[tooltipItem.datasetIndex].label + ": $ " + tooltipItem.yLabel;//092021 - number format required
                            }
                            /*,footer: (tooltipItems, data) => {
                                let total = tooltipItems.reduce((a, e) => a + parseFloat(e.yLabel), 0);
                                return 'Total: $ ' + total.toFixed(3);
	...
	'<input type="hidden" name="txtFldprojORCrtdBy"/><input type="hidden" name="txtFldprojORCrtdDate"/>'; 
		 ...
		 if($rowCount >0){
			 $("#OthRetPlgtbl tbody tr").each(function(i,row){
				 ...
				 var mode_r = $(this).find("td:first").find('input[name=txtFldothretMode]').val();
				var description = $(this).find("td:eq(2)").find('input[name=txtFldORtyofpay]').val();
				var annlExpnd =  $(this).find("td:eq(3)").find('input[name=txtFldORAnlExp]').val();
				var frequency = $(this).find("td:eq(3)").find("select[name=selORFreq]").val();
				var escrate = $(this).find("td:eq(3)").find("input[name=txtFldOREslrate]").val();
				var paystarts = $(this).find("td:eq(4)").find("input[name=txtFldORAgePaySts]").val();
				var paysends = $(this).find("td:eq(4)").find("input[name=txtFldORAgePayends]").val();
				var basedon = $(this).find("td:eq(4)").find("select[name=selORAgeBsOn]").val();
				var remarks =$(this).find("td:eq(5)").find("input[name=txtFldORRemarks]").val();
				
				frequency = (isEmpty(frequency)? "REGULAR" : frequency)
				annlExpnd = (isEmpty(annlExpnd)? Number("0") : Number(annlExpnd))
				escrate = (isEmpty(escrate)? Number("0") : Number(escrate))
				paystarts = (isEmpty(paystarts)? intretslfage : Number(paystarts))
				paysends = (isEmpty(paysends)? totAge : Number(paysends))
...
//				 if($rowCount >0){
				 if(mode_r != "D"){
						...
						$lastRow.find("td:eq(1)").find("input:first").click(function(){
							...
							$lastRow.find("td:eq(2)").find('input:eq(0)').val(description);
							
							$lastRow.find("td:eq(3)").find('select:eq(0)').append(orfreq);
							$lastRow.find("td:eq(3)").find('select:eq(0)').val(frequency);
							
							$lastRow.find("td:eq(4)").find('input:eq(0)').val(annlExpnd).addClass("applyEvntUsd");
							$lastRow.find("td:eq(5)").find('input:eq(0)').val(escrate).addClass("applyEvntpCent3");
							
							$lastRow.find("td:eq(6)").find('select:eq(0)').append(oragebsed);
							$lastRow.find("td:eq(6)").find('select:eq(0)').val(basedon);
							
							$lastRow.find("td:eq(7)").find('input:eq(0)').val(paystarts).addClass("applyEvntYrs");
							
							$lastRow.find("td:eq(8)").find('input:eq(0)').val(paysends).addClass("applyEvntYrs");
							
							$lastRow.find("td:eq(9)").find('input:eq(0)').val(remarks);
...
/*Auto complete*/
//is it working or any commented --092021
...
//15032021    Retirement Cash Flow Analysis

//common component pdf
$("#btnRetirementPlanningScreenPrint").click(function (){ 
	...
	var fileSave ="RP_CF_";
	var file = "RP_CF_";
	var screentittlefootername ="Retirement Cash Flow Analysis";
	//var divid0 ='rpt0'; //test self,spouse name
	var rpt1divid ='rpt1'; //div id 1  //image
	var cpfpayoutRDtabId ="cpfpayoutRD";  //table id 2 //1 'RDInctbl','RDIncAsstbl'
	var RDExptblId ="RDExptbl"; //table id 3  
	var generateproExptblId ="generateproExptbl"; //table id 4  
	var rpcfanaexpndchartId ='rpcfana_expndchart'; // chart id 5   //image
	var RDInctblId ="RDInctbl"; //table id 6
	var RDIncAsstblId ="RDIncAsstbl"; //table id 7
	var generateproIncometblId ="generateproIncometbl"; //table id 8
	var rpcfanaincomechartId ='rpcfana_incomechart'; //chart id 9  //image
	var rpt5didid1 ='rpt5'; //form div id 10  +   //image
	var generatetoamendincExptblId ="generatetoamendincExp"; //table id 11
	var rpcfanaannlexpincchartId ='rpcfana_annlexpinc'; //chart id 12   (2 chart)  //image
	var generatetoamendincExpAccmtblId ="generatetoamendincExpAccm"; //table id 13
	var rpcfanaannlexpincaccmchartId ='rpcfana_annlexpinc_accm'; //chart id 14 (2 chart)  //image
	
	var screentittle =""; //Retirement Cash Flow Analysis;  //screen title
	var pdfTitle ="CPF Life Pay Out Income"; // table title name page 2
	var pdfTitle1 ="Other payment to be made during retirement";  // table title name page 3
	//var pdfTitle2 ="Projected Expenditure Details on retirement";  // table title name page 4
	var pdfTitle2 ="Total Projected Expenditure Details on retirement";
	var chartTitle ="Projection of Expenditure on Retirement"; //chart title name page 5
	//var pdfTitle3 ="Income to be received during retirement"; // table title name page 6
	var pdfTitle3 ="Income and funds to be received on retirement";
	var pdfTitle4 ="Income and assets available for retirement"; // table title name page 7
	var pdfTitle5 ="Projected Income avaliable on retirement"; // table title name page 8
	var chartTitle1 ="Projection of Income"; //chart title name page 9
	var pdfTitle6 ="Annual Cashflow Projections of Retirement Income and Expenditure (Annual Based)"; // table title name page 10
	var chartTitle2 ="Cashflow Projection on Retirement"; //chart title name page 11
	var pdfTitle7 ="Annual Cashflow Projections of Retirement Income and Expenditure (Accumlation Based)"; // table title name page 12
	var chartTitle3 ="Cashflow Projection (Accumlation based)"; // chart title name page 13
	
	//tabId  //CPF Life Pay Out Income (header)
	var columnscpfpayoutRD =[];
	$("#cpfpayoutRD thead tr:first th").each(function(i){
		//if(i==1)return;
		var hdrText=$(this).find(">div").find('div').text();
		columnscpfpayoutRD.push(h2pcapitalizeWords(hdrText));
	});
	 var columnscpfpayoutRDAry =[]; //non group header we should follow this
	 columnscpfpayoutRDAry.push(columnscpfpayoutRD);

		//tabId1  //Other payment to be made during retirement (header)
		var columnsRDExptbl =[];
		$("#RDExptbl thead tr:first th").each(function(i){
			if(i==1)return;
			var hdrText=$(this).find(">div").find('div').text();
			columnsRDExptbl.push(h2pcapitalizeWords(hdrText));
		});
		 var columnsRDExptblAry =[]; //non group header we should follow this
		 columnsRDExptblAry.push(columnsRDExptbl);
	 
//tabId2  //Projected Expenditure Details on retirement (header)
	var columnsgenerateproExptbl =[];
	$("#generateproExptbl thead tr:first th").each(function(){//th:lt(14)
		var hdrText=$(this).find("div").text();
		columnsgenerateproExptbl.push(h2pcapitalizeWords(hdrText));
	});
	 var columnsgenerateproExptblAry =[]; //non group header we should follow this
	 columnsgenerateproExptblAry.push(columnsgenerateproExptbl);
	
	//tabId3  // Income to be received during retirement  (header)
	 var columnsRDInctbl =[];
	 $("#RDInctbl thead tr:first th").each(function(i){
		if(i==1)return;
		var hdrText=$(this).find(">div").find('div').text();
		columnsRDInctbl.push(h2pcapitalizeWords(hdrText));
	});
	 var columnsRDInctblAry =[]; //non group header we should follow this
	 columnsRDInctblAry.push(columnsRDInctbl);
	
	//tabId4   //Income and assets available for retirement  (header)
	var columnsRDIncAsstbl =[];
	$("#RDIncAsstbl thead tr:first th").each(function(i){
	if(i==1)return;
	var hdrText=$(this).find(">div").find('div').text();
	columnsRDIncAsstbl.push(h2pcapitalizeWords(hdrText));
	});
	
	var columnsRDIncAsstblAry =[]; //non group header we should follow this
	columnsRDIncAsstblAry.push(columnsRDIncAsstbl);
	
	//tabId5   //Projected Income avaliable on retirement  (header)
	var columnsgenerateproIncometbl =[];
	$("#generateproIncometbl thead tr:first th").each(function(){//th:lt(14)
	var hdrText=$(this).find("div").text();
	columnsgenerateproIncometbl.push(h2pcapitalizeWords(hdrText));
	});
	
	var columnsgenerateproIncometblAry =[]; //non group header we should follow this
	columnsgenerateproIncometblAry.push(columnsgenerateproIncometbl);
	
	//tabId6   // Annual Cashflow Projections of Retirement Income and Expenditure (Annual Based)   (header)
	...
	//tabId7    Annual Cashflow Projections of Retirement Income and Expenditure (Accumlation Based)  (header)
	var columnsgeneratetoamendincExpAccm =[];
	$("#generatetoamendincExpAccm thead tr:first th").each(function(){
	var hdrText=$(this).find("div").text();
	columnsgeneratetoamendincExpAccm.push(h2pcapitalizeWords(hdrText));
	});
	 
	var columnsgeneratetoamendincExpAccmAry =[]; //non group header we should follow this
	columnsgeneratetoamendincExpAccmAry.push(columnsgeneratetoamendincExpAccm);
	 		
	var jsonObject = 
	    [{"key":"Image","value":rpt1divid,"Title":screentittle,"TitleFooter":screentittlefootername,"header":"","pdfaddnewPage":""},
	    // {"key":"Table","value":cpfpayoutRDtabId,"Title":pdfTitle,"TitleFooter":screentittlefootername,"header":columnscpfpayoutRDAry,"pdfaddnewPage":"yes"},  //colheaderarray
	     {"key":"Table","value":RDExptblId,"Title":pdfTitle1,"TitleFooter":screentittlefootername,"header":columnsRDExptblAry,"pdfaddnewPage":"yes"}, //colheaderarray1
	     {"key":"Table","value":generateproExptblId,"Title":pdfTitle2,"TitleFooter":screentittlefootername,"header":columnsgenerateproExptblAry,"pdfaddnewPage":"yes"},
	     {"key":"Image","value":rpcfanaexpndchartId,"Title":chartTitle,"TitleFooter":screentittlefootername,"header":"","pdfaddnewPage":"yes"},
	     {"key":"Table","value":RDInctblId,"Title":pdfTitle3,"TitleFooter":screentittlefootername,"header":columnsRDInctblAry,"pdfaddnewPage":"yes"},
	     {"key":"Table","value":RDIncAsstblId,"Title":pdfTitle4,"TitleFooter":screentittlefootername,"header":columnsRDIncAsstblAry,"pdfaddnewPage":"yes"},
	     {"key":"Table","value":generateproIncometblId,"Title":pdfTitle5,"TitleFooter":screentittlefootername,"header":columnsgenerateproIncometblAry,"pdfaddnewPage":"yes"},
	     {"key":"Image","value":rpcfanaincomechartId,"Title":chartTitle1,"TitleFooter":screentittlefootername,"header":"","pdfaddnewPage":"yes"},
	     {"key":"Image","value":rpt5didid1,"Title":pdfTitle6,"TitleFooter":screentittlefootername,"header":"","pdfaddnewPage":"yes"},
	     {"key":"Table","value":generatetoamendincExptblId,"Title":pdfTitle6,"TitleFooter":screentittlefootername,"header":columnsgeneratetoamendincExpAry,"pdfaddnewPage":"yes"},
	     {"key":"Image","value":rpcfanaannlexpincchartId,"Title":chartTitle2,"TitleFooter":screentittlefootername,"header":"","pdfaddnewPage":"yes"},
	     {"key":"Table","value":generatetoamendincExpAccmtblId,"Title":pdfTitle7,"TitleFooter":screentittlefootername,"header":columnsgeneratetoamendincExpAccmAry,"pdfaddnewPage":"yes"}
	    // {"key":"Image","value":rpcfanaannlexpincaccmchartId,"Title":chartTitle3,"TitleFooter":screentittlefootername,"header":"","pdfaddnewPage":"yes"}
 ];
	...
	var objArrayfilename = JSON.parse(jsonArray); // parse jsonArray to variable(objArray)
	 ...
	 setTimeout(function(){
		 ...
		 h2pcompfun(jsonObject,objArrayfilename,file); //function to start print pdf
...
$("#btnAccumlationBasedPrint").click(function (){ 
	
	 
	  
	var pl ="landscape";  // landscape     portrait
	var fileSave ="RP_CF_Accumulated";
	var file = "RP_CF_Accumulated";
	var screentittlefootername ="Retirement Cash Flow Analysis";
	...
	var jsonObject = 
	    [   {"key":"Table","value":generatetoamendincExpAccmtblId,"Title":pdfTitle7,"TitleFooter":screentittlefootername,"header":columnsgeneratetoamendincExpAccmAry,"pdfaddnewPage":""},
	    {"key":"Image","value":rpcfanaannlexpincaccmchartId,"Title":chartTitle3,"TitleFooter":screentittlefootername,"header":"","pdfaddnewPage":"yes"}
 ];
